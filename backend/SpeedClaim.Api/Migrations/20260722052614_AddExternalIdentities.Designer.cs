// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;
using SpeedClaim.Api.Context;

#nullable disable

namespace SpeedClaim.Api.Migrations
{
    [DbContext(typeof(SpeedClaimDbContext))]
    [Migration("20260722052614_AddExternalIdentities")]
    partial class AddExternalIdentities
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "10.0.0-preview.1.25081.1")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("SpeedClaim.Api.Models.Address", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("AddressLine1")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("address_line1");

                    b.Property<string>("AddressLine2")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("address_line2");

                    b.Property<string>("AddressType")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("address_type");

                    b.Property<string>("City")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("city");

                    b.Property<string>("Country")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("country");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<bool>("IsSameAsPermanent")
                        .HasColumnType("boolean")
                        .HasColumnName("is_same_as_permanent");

                    b.Property<string>("Pincode")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasColumnName("pincode");

                    b.Property<string>("State")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("state");

                    b.Property<DateTimeOffset?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid")
                        .HasColumnName("user_id");

                    b.HasKey("Id")
                        .HasName("PK_addresses");

                    b.HasIndex("UserId");

                    b.ToTable("addresses");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.Agent", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("AgentCode")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasColumnName("agent_code");

                    b.Property<string>("AgentType")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("agent_type");

                    b.Property<Guid?>("BranchId")
                        .HasColumnType("uuid")
                        .HasColumnName("branch_id");

                    b.Property<decimal>("CommissionRate")
                        .HasColumnType("decimal(5,2)")
                        .HasColumnName("commission_rate");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean")
                        .HasColumnName("is_active");

                    b.Property<DateOnly>("LicenseExpiry")
                        .HasColumnType("date")
                        .HasColumnName("license_expiry");

                    b.Property<string>("LicenseNumber")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("license_number");

                    b.Property<DateTimeOffset?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid")
                        .HasColumnName("user_id");

                    b.HasKey("Id")
                        .HasName("PK_agents");

                    b.HasIndex("BranchId");

                    b.HasIndex("UserId")
                        .IsUnique()
                        .HasDatabaseName("uq_agents_user_id");

                    b.ToTable("agents");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.AgentCommission", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid>("AgentId")
                        .HasColumnType("uuid")
                        .HasColumnName("agent_id");

                    b.Property<decimal>("CommissionAmount")
                        .HasColumnType("decimal(10,2)")
                        .HasColumnName("commission_amount");

                    b.Property<decimal>("CommissionRate")
                        .HasColumnType("decimal(5,2)")
                        .HasColumnName("commission_rate");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<DateTimeOffset?>("PaidAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("paid_at");

                    b.Property<Guid>("PolicyId")
                        .HasColumnType("uuid")
                        .HasColumnName("policy_id");

                    b.Property<Guid>("PremiumPaymentId")
                        .HasColumnType("uuid")
                        .HasColumnName("premium_payment_id");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("status");

                    b.HasKey("Id")
                        .HasName("PK_agent_commissions");

                    b.HasIndex("AgentId");

                    b.HasIndex("PolicyId");

                    b.HasIndex("PremiumPaymentId");

                    b.ToTable("agent_commissions");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.AuditLog", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("Action")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("action");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<Guid>("EntityId")
                        .HasColumnType("uuid")
                        .HasColumnName("entity_id");

                    b.Property<string>("EntityType")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("entity_type");

                    b.Property<string>("IpAddress")
                        .HasMaxLength(45)
                        .HasColumnType("character varying(45)")
                        .HasColumnName("ip_address");

                    b.Property<string>("NewValue")
                        .HasColumnType("jsonb")
                        .HasColumnName("new_value");

                    b.Property<string>("OldValue")
                        .HasColumnType("jsonb")
                        .HasColumnName("old_value");

                    b.Property<Guid?>("UserId")
                        .HasColumnType("uuid")
                        .HasColumnName("user_id");

                    b.HasKey("Id")
                        .HasName("PK_audit_logs");

                    b.HasIndex("UserId");

                    b.ToTable("audit_logs");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.Branch", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("Address")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("address");

                    b.Property<string>("City")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("city");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)")
                        .HasColumnName("email");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean")
                        .HasColumnName("is_active");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("name");

                    b.Property<string>("Phone")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasColumnName("phone");

                    b.Property<string>("State")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("state");

                    b.Property<DateTimeOffset?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.HasKey("Id")
                        .HasName("PK_branches");

                    b.ToTable("branches");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.Claim", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid?>("AssignedOfficerId")
                        .HasColumnType("uuid")
                        .HasColumnName("assigned_officer_id");

                    b.Property<decimal?>("ClaimAmountApproved")
                        .HasColumnType("decimal(15,2)")
                        .HasColumnName("claim_amount_approved");

                    b.Property<decimal>("ClaimAmountRequested")
                        .HasColumnType("decimal(15,2)")
                        .HasColumnName("claim_amount_requested");

                    b.Property<string>("ClaimNumber")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("character varying(30)")
                        .HasColumnName("claim_number");

                    b.Property<string>("ClaimType")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("claim_type");

                    b.Property<Guid?>("ClaimantMemberId")
                        .HasColumnType("uuid")
                        .HasColumnName("claimant_member_id");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<Guid>("CustomerId")
                        .HasColumnType("uuid")
                        .HasColumnName("customer_id");

                    b.Property<DateTimeOffset?>("DeletedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("deleted_at");

                    b.Property<DateTime>("IncidentDate")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("incident_date");

                    b.Property<string>("IncidentDescription")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("incident_description");

                    b.Property<DateTime>("IntimationDate")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("intimation_date");

                    b.Property<bool>("IsCashless")
                        .HasColumnType("boolean")
                        .HasColumnName("is_cashless");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("boolean")
                        .HasColumnName("is_deleted");

                    b.Property<Guid>("PolicyId")
                        .HasColumnType("uuid")
                        .HasColumnName("policy_id");

                    b.Property<string>("RejectionReason")
                        .HasColumnType("text")
                        .HasColumnName("rejection_reason");

                    b.Property<DateTime?>("SettlementDate")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("settlement_date");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("status");

                    b.Property<Guid?>("SurveyorId")
                        .HasColumnType("uuid")
                        .HasColumnName("surveyor_id");

                    b.Property<DateTimeOffset?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.HasKey("Id")
                        .HasName("PK_claims");

                    b.HasIndex("AssignedOfficerId");

                    b.HasIndex("ClaimNumber")
                        .IsUnique()
                        .HasDatabaseName("uq_claims_claim_number");

                    b.HasIndex("ClaimantMemberId");

                    b.HasIndex("CustomerId");

                    b.HasIndex("PolicyId");

                    b.HasIndex("SurveyorId");

                    b.ToTable("claims");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.ClaimStatusHistory", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTimeOffset>("ChangedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("changed_at");

                    b.Property<Guid?>("ChangedById")
                        .HasColumnType("uuid")
                        .HasColumnName("changed_by_id");

                    b.Property<Guid>("ClaimId")
                        .HasColumnType("uuid")
                        .HasColumnName("claim_id");

                    b.Property<string>("NewStatus")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("new_status");

                    b.Property<string>("Notes")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("notes");

                    b.Property<string>("OldStatus")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("old_status");

                    b.HasKey("Id")
                        .HasName("PK_claim_status_history");

                    b.HasIndex("ChangedById");

                    b.HasIndex("ClaimId");

                    b.ToTable("claim_status_histories");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.Customer", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<decimal>("AnnualIncome")
                        .HasColumnType("decimal(15,2)")
                        .HasColumnName("annual_income");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<DateOnly?>("DateOfBirth")
                        .HasColumnType("date")
                        .HasColumnName("date_of_birth");

                    b.Property<string>("Gender")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("gender");

                    b.Property<string>("MaritalStatus")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("marital_status");

                    b.Property<string>("Occupation")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("occupation");

                    b.Property<Guid?>("OnboardingAgentId")
                        .HasColumnType("uuid")
                        .HasColumnName("onboarding_agent_id");

                    b.Property<DateTimeOffset?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid")
                        .HasColumnName("user_id");

                    b.HasKey("Id")
                        .HasName("PK_customers");

                    b.HasIndex("OnboardingAgentId");

                    b.HasIndex("UserId")
                        .IsUnique();

                    b.ToTable("customers");

                    b.HasData(
                        new
                        {
                            Id = new Guid("22222222-2222-2222-2222-222222222222"),
                            AnnualIncome = 150000m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 6, 7, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            DateOfBirth = new DateOnly(1995, 1, 1),
                            Gender = "Male",
                            MaritalStatus = "Single",
                            Occupation = "Software Engineer",
                            UserId = new Guid("11111111-1111-1111-1111-111111111111")
                        });
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.CustomerMember", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<Guid>("CustomerId")
                        .HasColumnType("uuid")
                        .HasColumnName("customer_id");

                    b.Property<DateOnly>("DateOfBirth")
                        .HasColumnType("date")
                        .HasColumnName("date_of_birth");

                    b.Property<string>("FirstName")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("first_name");

                    b.Property<string>("Gender")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("gender");

                    b.Property<bool>("IsDependent")
                        .HasColumnType("boolean")
                        .HasColumnName("is_dependent");

                    b.Property<string>("LastName")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("last_name");

                    b.Property<string>("Relationship")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("relationship");

                    b.Property<string>("Salutation")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasColumnName("salutation");

                    b.Property<DateTimeOffset?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.HasKey("Id")
                        .HasName("PK_customer_members");

                    b.HasIndex("CustomerId");

                    b.ToTable("customer_members");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.DocumentRequirement", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("description");

                    b.Property<string>("DocumentKey")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("document_key");

                    b.Property<string>("Domain")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("domain");

                    b.Property<string>("EntityType")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("entity_type");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean")
                        .HasColumnName("is_active");

                    b.Property<bool>("IsMandatory")
                        .HasColumnType("boolean")
                        .HasColumnName("is_mandatory");

                    b.Property<string>("Label")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("label");

                    b.Property<Guid?>("ProductId")
                        .HasColumnType("uuid")
                        .HasColumnName("product_id");

                    b.HasKey("Id")
                        .HasName("PK_document_requirements");

                    b.HasIndex("ProductId");

                    b.ToTable("document_requirements");

                    b.HasData(
                        new
                        {
                            Id = new Guid("10000000-0000-0000-0000-000000000001"),
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 6, 7, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            Description = "",
                            DocumentKey = "AADHAAR",
                            Domain = "ALL",
                            EntityType = "Kyc",
                            IsActive = true,
                            IsMandatory = false,
                            Label = "Aadhaar Card"
                        },
                        new
                        {
                            Id = new Guid("10000000-0000-0000-0000-000000000002"),
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 6, 7, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            Description = "",
                            DocumentKey = "PAN",
                            Domain = "ALL",
                            EntityType = "Kyc",
                            IsActive = true,
                            IsMandatory = false,
                            Label = "PAN Card"
                        },
                        new
                        {
                            Id = new Guid("71200000-0000-0000-0000-000000000001"),
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            Description = "Government-issued photo identity document.",
                            DocumentKey = "GOVERNMENT_ID",
                            Domain = "Health",
                            EntityType = "Proposal",
                            IsActive = true,
                            IsMandatory = true,
                            Label = "Government photo ID",
                            ProductId = new Guid("71000000-0000-0000-0000-000000000001")
                        },
                        new
                        {
                            Id = new Guid("71200000-0000-0000-0000-000000000002"),
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            Description = "Document confirming the proposer’s age.",
                            DocumentKey = "AGE_PROOF",
                            Domain = "Health",
                            EntityType = "Proposal",
                            IsActive = true,
                            IsMandatory = true,
                            Label = "Age proof",
                            ProductId = new Guid("71000000-0000-0000-0000-000000000001")
                        },
                        new
                        {
                            Id = new Guid("71200000-0000-0000-0000-000000000003"),
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            Description = "Completed health and medical declaration.",
                            DocumentKey = "MEDICAL_DECLARATION",
                            Domain = "Health",
                            EntityType = "Proposal",
                            IsActive = true,
                            IsMandatory = true,
                            Label = "Medical declaration",
                            ProductId = new Guid("71000000-0000-0000-0000-000000000001")
                        },
                        new
                        {
                            Id = new Guid("71200000-0000-0000-0000-000000000004"),
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            Description = "Required only when porting an existing health policy.",
                            DocumentKey = "PREVIOUS_POLICY",
                            Domain = "Health",
                            EntityType = "Proposal",
                            IsActive = true,
                            IsMandatory = false,
                            Label = "Previous policy copy",
                            ProductId = new Guid("71000000-0000-0000-0000-000000000001")
                        },
                        new
                        {
                            Id = new Guid("71200000-0000-0000-0000-000000000005"),
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            Description = "Government-issued photo identity document.",
                            DocumentKey = "GOVERNMENT_ID",
                            Domain = "Life",
                            EntityType = "Proposal",
                            IsActive = true,
                            IsMandatory = true,
                            Label = "Government photo ID",
                            ProductId = new Guid("71000000-0000-0000-0000-000000000002")
                        },
                        new
                        {
                            Id = new Guid("71200000-0000-0000-0000-000000000006"),
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            Description = "PAN or other permitted tax identity document.",
                            DocumentKey = "PAN",
                            Domain = "Life",
                            EntityType = "Proposal",
                            IsActive = true,
                            IsMandatory = true,
                            Label = "PAN or tax ID",
                            ProductId = new Guid("71000000-0000-0000-0000-000000000002")
                        },
                        new
                        {
                            Id = new Guid("71200000-0000-0000-0000-000000000007"),
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            Description = "Required for requested cover above INR 50 lakh.",
                            DocumentKey = "INCOME_PROOF",
                            Domain = "Life",
                            EntityType = "Proposal",
                            IsActive = true,
                            IsMandatory = false,
                            Label = "Income proof",
                            ProductId = new Guid("71000000-0000-0000-0000-000000000002")
                        },
                        new
                        {
                            Id = new Guid("71200000-0000-0000-0000-000000000008"),
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            Description = "Required only when requested during underwriting.",
                            DocumentKey = "MEDICAL_TESTS",
                            Domain = "Life",
                            EntityType = "Proposal",
                            IsActive = true,
                            IsMandatory = false,
                            Label = "Medical tests",
                            ProductId = new Guid("71000000-0000-0000-0000-000000000002")
                        },
                        new
                        {
                            Id = new Guid("71200000-0000-0000-0000-000000000009"),
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            Description = "Vehicle registration certificate (RC).",
                            DocumentKey = "REGISTRATION_CERTIFICATE",
                            Domain = "Motor",
                            EntityType = "Proposal",
                            IsActive = true,
                            IsMandatory = true,
                            Label = "Registration certificate",
                            ProductId = new Guid("71000000-0000-0000-0000-000000000003")
                        },
                        new
                        {
                            Id = new Guid("71200000-0000-0000-0000-000000000010"),
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            Description = "Valid driving licence of the primary driver.",
                            DocumentKey = "DRIVING_LICENCE",
                            Domain = "Motor",
                            EntityType = "Proposal",
                            IsActive = true,
                            IsMandatory = true,
                            Label = "Valid driving licence",
                            ProductId = new Guid("71000000-0000-0000-0000-000000000003")
                        },
                        new
                        {
                            Id = new Guid("71200000-0000-0000-0000-000000000011"),
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            Description = "Required where there is prior motor insurance coverage.",
                            DocumentKey = "PREVIOUS_POLICY",
                            Domain = "Motor",
                            EntityType = "Proposal",
                            IsActive = true,
                            IsMandatory = false,
                            Label = "Existing policy copy",
                            ProductId = new Guid("71000000-0000-0000-0000-000000000003")
                        },
                        new
                        {
                            Id = new Guid("71200000-0000-0000-0000-000000000012"),
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            Description = "Required when inspection is requested.",
                            DocumentKey = "VEHICLE_PHOTOS",
                            Domain = "Motor",
                            EntityType = "Proposal",
                            IsActive = true,
                            IsMandatory = false,
                            Label = "Vehicle photographs",
                            ProductId = new Guid("71000000-0000-0000-0000-000000000003")
                        });
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.EmailLog", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("ErrorMessage")
                        .HasColumnType("text")
                        .HasColumnName("error_message");

                    b.Property<string>("ProviderMessageId")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("provider_message_id");

                    b.Property<string>("RecipientEmail")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)")
                        .HasColumnName("recipient_email");

                    b.Property<DateTimeOffset?>("SentAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("sent_at");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("status");

                    b.Property<string>("Subject")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("subject");

                    b.Property<string>("TemplateKey")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("template_key");

                    b.Property<Guid?>("UserId")
                        .HasColumnType("uuid")
                        .HasColumnName("user_id");

                    b.Property<string>("VariablesUsed")
                        .HasColumnType("jsonb")
                        .HasColumnName("variables_used");

                    b.HasKey("Id")
                        .HasName("PK_email_logs");

                    b.HasIndex("UserId");

                    b.ToTable("email_logs");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.EmailTemplate", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("BodyHtml")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("body_html");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean")
                        .HasColumnName("is_active");

                    b.Property<string>("Subject")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("subject");

                    b.Property<string>("TemplateKey")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("template_key");

                    b.Property<DateTimeOffset?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.HasKey("Id")
                        .HasName("PK_email_templates");

                    b.ToTable("email_templates");

                    b.HasData(
                        new
                        {
                            Id = new Guid("dd000001-0000-0000-0000-000000000001"),
                            BodyHtml = "<!DOCTYPE html><html lang=\"en\"><head><meta charset=\"UTF-8\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\"><title>Verify your email</title></head><body style=\"margin:0;padding:0;background-color:#F7F9FA;font-family:Arial,'Helvetica Neue',Helvetica,sans-serif;\"><table role=\"presentation\" width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" style=\"background-color:#F7F9FA;padding:32px 16px;\"><tr><td align=\"center\"><table role=\"presentation\" width=\"480\" cellpadding=\"0\" cellspacing=\"0\" style=\"max-width:480px;width:100%;background-color:#ffffff;border-radius:12px;overflow:hidden;\"><tr><td style=\"background:linear-gradient(160deg,#0F6E8C 0%,#0A3040 100%);padding:28px 32px;text-align:center;\"><img src=\"{{logoUrl}}\" width=\"28\" height=\"28\" alt=\"SpeedClaim\" style=\"vertical-align:middle;margin-right:8px;border-radius:6px;\" /><span style=\"font-size:20px;font-weight:700;color:#ffffff;letter-spacing:-0.3px;vertical-align:middle;\">SpeedClaim</span></td></tr><tr><td style=\"padding:32px;\"><h1 style=\"margin:0 0 16px;font-size:22px;font-weight:600;color:#1A2230;\">Verify your email</h1><p style=\"margin:0 0 28px;font-size:15px;line-height:1.6;color:#3C4654;\">Welcome to SpeedClaim! Please verify your email address to activate your account and start managing your insurance policies.</p><table role=\"presentation\" cellpadding=\"0\" cellspacing=\"0\" width=\"100%\"><tr><td align=\"center\"><table role=\"presentation\" cellpadding=\"0\" cellspacing=\"0\"><tr><td style=\"background-color:#0F6E8C;border-radius:8px;\"><a href=\"{{verifyUrl}}\" target=\"_blank\" style=\"display:inline-block;padding:14px 36px;font-size:14px;font-weight:600;color:#ffffff;text-decoration:none;\">Verify Email</a></td></tr></table></td></tr></table><p style=\"margin:28px 0 0;font-size:13px;line-height:1.5;color:#6B7685;\">This link expires in 24 hours. If you didn't create an account, you can safely ignore this email.</p></td></tr><tr><td style=\"padding:20px 32px;border-top:1px solid #E2E6EB;text-align:center;\"><p style=\"margin:0;font-size:12px;color:#6B7685;\">&copy; {{year}} SpeedClaim. All rights reserved.</p></td></tr></table></td></tr></table></body></html>",
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 6, 7, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            IsActive = true,
                            Subject = "Verify Your SpeedClaim Account",
                            TemplateKey = "EmailVerification"
                        },
                        new
                        {
                            Id = new Guid("dd000002-0000-0000-0000-000000000002"),
                            BodyHtml = "<!DOCTYPE html><html lang=\"en\"><head><meta charset=\"UTF-8\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\"><title>Reset your password</title></head><body style=\"margin:0;padding:0;background-color:#F7F9FA;font-family:Arial,'Helvetica Neue',Helvetica,sans-serif;\"><table role=\"presentation\" width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" style=\"background-color:#F7F9FA;padding:32px 16px;\"><tr><td align=\"center\"><table role=\"presentation\" width=\"480\" cellpadding=\"0\" cellspacing=\"0\" style=\"max-width:480px;width:100%;background-color:#ffffff;border-radius:12px;overflow:hidden;\"><tr><td style=\"background:linear-gradient(160deg,#0F6E8C 0%,#0A3040 100%);padding:28px 32px;text-align:center;\"><img src=\"{{logoUrl}}\" width=\"28\" height=\"28\" alt=\"SpeedClaim\" style=\"vertical-align:middle;margin-right:8px;border-radius:6px;\" /><span style=\"font-size:20px;font-weight:700;color:#ffffff;letter-spacing:-0.3px;vertical-align:middle;\">SpeedClaim</span></td></tr><tr><td style=\"padding:32px;\"><h1 style=\"margin:0 0 16px;font-size:22px;font-weight:600;color:#1A2230;\">Reset your password</h1><p style=\"margin:0 0 28px;font-size:15px;line-height:1.6;color:#3C4654;\">We received a request to reset your password. Click the button below to choose a new one.</p><table role=\"presentation\" cellpadding=\"0\" cellspacing=\"0\" width=\"100%\"><tr><td align=\"center\"><table role=\"presentation\" cellpadding=\"0\" cellspacing=\"0\"><tr><td style=\"background-color:#0F6E8C;border-radius:8px;\"><a href=\"{{resetUrl}}\" target=\"_blank\" style=\"display:inline-block;padding:14px 36px;font-size:14px;font-weight:600;color:#ffffff;text-decoration:none;\">Reset Password</a></td></tr></table></td></tr></table><p style=\"margin:28px 0 0;font-size:13px;line-height:1.5;color:#6B7685;\">This link expires in 1 hour. If you didn't request a password reset, you can safely ignore this email.</p></td></tr><tr><td style=\"padding:20px 32px;border-top:1px solid #E2E6EB;text-align:center;\"><p style=\"margin:0;font-size:12px;color:#6B7685;\">&copy; {{year}} SpeedClaim. All rights reserved.</p></td></tr></table></td></tr></table></body></html>",
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 6, 7, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            IsActive = true,
                            Subject = "Reset Your SpeedClaim Password",
                            TemplateKey = "PasswordReset"
                        },
                        new
                        {
                            Id = new Guid("dd000003-0000-0000-0000-000000000003"),
                            BodyHtml = "<!DOCTYPE html><html lang=\"en\"><head><meta charset=\"UTF-8\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\"><title>Policy Activated</title></head><body style=\"margin:0;padding:0;background-color:#F4F7FA;font-family:Arial,Helvetica,sans-serif;color:#1A2230;\"><table role=\"presentation\" width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" style=\"background-color:#F4F7FA;padding:28px 12px;\"><tr><td align=\"center\"><table role=\"presentation\" width=\"640\" cellpadding=\"0\" cellspacing=\"0\" style=\"max-width:640px;width:100%;background:#ffffff;border:1px solid #E2E6EB;border-radius:14px;overflow:hidden;\"><tr><td style=\"background:#0F6E8C;padding:24px 30px;\"><table role=\"presentation\" cellpadding=\"0\" cellspacing=\"0\"><tr><td style=\"vertical-align:middle;padding-right:8px;\"><img src=\"{{logoUrl}}\" width=\"30\" height=\"30\" alt=\"SpeedClaim\" style=\"display:block;border-radius:6px;\" /></td><td style=\"vertical-align:middle;\"><div style=\"font-size:22px;font-weight:700;color:#ffffff;\">SpeedClaim</div></td></tr></table><div style=\"font-size:13px;color:#D8EEF5;margin-top:4px;\">Insurance policy services</div></td></tr><tr><td style=\"padding:30px;\"><p style=\"margin:0 0 10px;font-size:14px;color:#1F9D6B;font-weight:700;\">Policy activated</p><h1 style=\"margin:0 0 14px;font-size:25px;line-height:1.25;color:#1A2230;\">Your cover is now active</h1><p style=\"margin:0 0 24px;font-size:15px;line-height:1.65;color:#3C4654;\">Dear {{firstName}}, your policy has been activated successfully. A PDF copy of your policy certificate is attached to this email for your records.</p><table role=\"presentation\" width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" style=\"border-collapse:collapse;border:1px solid #E2E6EB;border-radius:10px;overflow:hidden;\"><tr><td style=\"padding:13px 16px;background:#F7F9FA;font-size:12px;color:#6B7685;width:38%;\">Policy number</td><td style=\"padding:13px 16px;font-size:14px;font-weight:700;color:#1A2230;\">{{policyNumber}}</td></tr><tr><td style=\"padding:13px 16px;background:#F7F9FA;font-size:12px;color:#6B7685;\">Product</td><td style=\"padding:13px 16px;font-size:14px;color:#1A2230;\">{{product}}</td></tr><tr><td style=\"padding:13px 16px;background:#F7F9FA;font-size:12px;color:#6B7685;\">Coverage amount</td><td style=\"padding:13px 16px;font-size:14px;color:#1A2230;\">{{sumAssured}} INR</td></tr><tr><td style=\"padding:13px 16px;background:#F7F9FA;font-size:12px;color:#6B7685;\">Premium</td><td style=\"padding:13px 16px;font-size:14px;color:#1A2230;\">{{premiumAmount}} INR / {{frequency}}</td></tr><tr><td style=\"padding:13px 16px;background:#F7F9FA;font-size:12px;color:#6B7685;\">Policy period</td><td style=\"padding:13px 16px;font-size:14px;color:#1A2230;\">{{startDate}} - {{endDate}}</td></tr><tr><td style=\"padding:13px 16px;background:#F7F9FA;font-size:12px;color:#6B7685;\">Status</td><td style=\"padding:13px 16px;font-size:14px;font-weight:700;color:#1F9D6B;\">{{status}}</td></tr></table><p style=\"margin:24px 0 0;font-size:13px;line-height:1.55;color:#6B7685;\">Please review the attached certificate and keep it with your insurance records. Contact SpeedClaim support if any detail looks incorrect.</p></td></tr><tr><td style=\"padding:18px 30px;background:#F7F9FA;border-top:1px solid #E2E6EB;\"><p style=\"margin:0;font-size:12px;line-height:1.5;color:#6B7685;\">This is a system-generated message from SpeedClaim. &copy; {{year}} SpeedClaim.</p></td></tr></table></td></tr></table></body></html>",
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 6, 7, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            IsActive = true,
                            Subject = "Your SpeedClaim policy is active - {{policyNumber}}",
                            TemplateKey = "PolicyActivated"
                        },
                        new
                        {
                            Id = new Guid("dd000004-0000-0000-0000-000000000004"),
                            BodyHtml = "<!DOCTYPE html><html lang=\"en\"><head><meta charset=\"UTF-8\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\"><title>KYC Verified</title></head><body style=\"margin:0;padding:0;background-color:#F7F9FA;font-family:Arial,'Helvetica Neue',Helvetica,sans-serif;\"><table role=\"presentation\" width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" style=\"background-color:#F7F9FA;padding:32px 16px;\"><tr><td align=\"center\"><table role=\"presentation\" width=\"480\" cellpadding=\"0\" cellspacing=\"0\" style=\"max-width:480px;width:100%;background-color:#ffffff;border-radius:12px;overflow:hidden;\"><tr><td style=\"background:linear-gradient(160deg,#0F6E8C 0%,#0A3040 100%);padding:28px 32px;text-align:center;\"><img src=\"{{logoUrl}}\" width=\"28\" height=\"28\" alt=\"SpeedClaim\" style=\"vertical-align:middle;margin-right:8px;border-radius:6px;\" /><span style=\"font-size:20px;font-weight:700;color:#ffffff;letter-spacing:-0.3px;vertical-align:middle;\">SpeedClaim</span></td></tr><tr><td style=\"padding:32px;\"><span style=\"display:inline-block;margin:0 0 12px;padding:4px 10px;background:#E8F7F1;color:#1F9D6B;border-radius:20px;font-size:11px;font-weight:700;\">KYC Verified</span><h1 style=\"margin:0 0 16px;font-size:22px;font-weight:600;color:#1A2230;\">Your identity has been verified</h1><p style=\"margin:0;font-size:15px;line-height:1.6;color:#3C4654;\">Dear {{firstName}}, your KYC documents have been successfully reviewed and verified. You are now eligible to apply for insurance products and have policies issued.</p></td></tr><tr><td style=\"padding:20px 32px;border-top:1px solid #E2E6EB;text-align:center;\"><p style=\"margin:0;font-size:12px;color:#6B7685;\">&copy; {{year}} SpeedClaim. All rights reserved.</p></td></tr></table></td></tr></table></body></html>",
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 6, 7, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            IsActive = true,
                            Subject = "Your KYC has been verified - SpeedClaim",
                            TemplateKey = "KycApproved"
                        },
                        new
                        {
                            Id = new Guid("dd000005-0000-0000-0000-000000000005"),
                            BodyHtml = "<!DOCTYPE html><html lang=\"en\"><head><meta charset=\"UTF-8\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\"><title>KYC Rejected</title></head><body style=\"margin:0;padding:0;background-color:#F7F9FA;font-family:Arial,'Helvetica Neue',Helvetica,sans-serif;\"><table role=\"presentation\" width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" style=\"background-color:#F7F9FA;padding:32px 16px;\"><tr><td align=\"center\"><table role=\"presentation\" width=\"480\" cellpadding=\"0\" cellspacing=\"0\" style=\"max-width:480px;width:100%;background-color:#ffffff;border-radius:12px;overflow:hidden;\"><tr><td style=\"background:linear-gradient(160deg,#0F6E8C 0%,#0A3040 100%);padding:28px 32px;text-align:center;\"><img src=\"{{logoUrl}}\" width=\"28\" height=\"28\" alt=\"SpeedClaim\" style=\"vertical-align:middle;margin-right:8px;border-radius:6px;\" /><span style=\"font-size:20px;font-weight:700;color:#ffffff;letter-spacing:-0.3px;vertical-align:middle;\">SpeedClaim</span></td></tr><tr><td style=\"padding:32px;\"><span style=\"display:inline-block;margin:0 0 12px;padding:4px 10px;background:#FBE9E9;color:#D14343;border-radius:20px;font-size:11px;font-weight:700;\">KYC Rejected</span><h1 style=\"margin:0 0 16px;font-size:22px;font-weight:600;color:#1A2230;\">KYC verification unsuccessful</h1><p style=\"margin:0 0 16px;font-size:15px;line-height:1.6;color:#3C4654;\">Dear {{firstName}}, we were unable to verify your identity documents.</p><div style=\"margin:0 0 20px;padding:12px 16px;background:#FBE9E9;border-left:3px solid #D14343;border-radius:4px;\"><p style=\"margin:0 0 4px;font-size:12px;font-weight:700;color:#D14343;\">Reason</p><p style=\"margin:0;font-size:14px;color:#3C4654;\">{{rejectionReason}}</p></div><p style=\"margin:0;font-size:13px;line-height:1.5;color:#6B7685;\">Please log in to the customer portal to re-upload your documents.</p></td></tr><tr><td style=\"padding:20px 32px;border-top:1px solid #E2E6EB;text-align:center;\"><p style=\"margin:0;font-size:12px;color:#6B7685;\">&copy; {{year}} SpeedClaim. All rights reserved.</p></td></tr></table></td></tr></table></body></html>",
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 6, 7, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            IsActive = true,
                            Subject = "KYC verification unsuccessful - SpeedClaim",
                            TemplateKey = "KycRejected"
                        },
                        new
                        {
                            Id = new Guid("dd000006-0000-0000-0000-000000000006"),
                            BodyHtml = "<!DOCTYPE html><html lang=\"en\"><head><meta charset=\"UTF-8\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\"><title>Proposal Approved</title></head><body style=\"margin:0;padding:0;background-color:#F7F9FA;font-family:Arial,'Helvetica Neue',Helvetica,sans-serif;\"><table role=\"presentation\" width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" style=\"background-color:#F7F9FA;padding:32px 16px;\"><tr><td align=\"center\"><table role=\"presentation\" width=\"480\" cellpadding=\"0\" cellspacing=\"0\" style=\"max-width:480px;width:100%;background-color:#ffffff;border-radius:12px;overflow:hidden;\"><tr><td style=\"background:linear-gradient(160deg,#0F6E8C 0%,#0A3040 100%);padding:28px 32px;text-align:center;\"><img src=\"{{logoUrl}}\" width=\"28\" height=\"28\" alt=\"SpeedClaim\" style=\"vertical-align:middle;margin-right:8px;border-radius:6px;\" /><span style=\"font-size:20px;font-weight:700;color:#ffffff;letter-spacing:-0.3px;vertical-align:middle;\">SpeedClaim</span></td></tr><tr><td style=\"padding:32px;\"><span style=\"display:inline-block;margin:0 0 12px;padding:4px 10px;background:#E8F7F1;color:#1F9D6B;border-radius:20px;font-size:11px;font-weight:700;\">Proposal Approved</span><h1 style=\"margin:0 0 16px;font-size:22px;font-weight:600;color:#1A2230;\">Your proposal has been approved</h1><p style=\"margin:0;font-size:15px;line-height:1.6;color:#3C4654;\">Dear {{firstName}}, your insurance proposal <strong>{{proposalNumber}}</strong> has been reviewed and approved by our underwriting team. A policy has been created and will activate once your first premium payment is received. Please log in to complete your payment.</p></td></tr><tr><td style=\"padding:20px 32px;border-top:1px solid #E2E6EB;text-align:center;\"><p style=\"margin:0;font-size:12px;color:#6B7685;\">&copy; {{year}} SpeedClaim. All rights reserved.</p></td></tr></table></td></tr></table></body></html>",
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 6, 7, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            IsActive = true,
                            Subject = "Your proposal {{proposalNumber}} has been approved",
                            TemplateKey = "ProposalApproved"
                        },
                        new
                        {
                            Id = new Guid("dd000007-0000-0000-0000-000000000007"),
                            BodyHtml = "<!DOCTYPE html><html lang=\"en\"><head><meta charset=\"UTF-8\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\"><title>Proposal Rejected</title></head><body style=\"margin:0;padding:0;background-color:#F7F9FA;font-family:Arial,'Helvetica Neue',Helvetica,sans-serif;\"><table role=\"presentation\" width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" style=\"background-color:#F7F9FA;padding:32px 16px;\"><tr><td align=\"center\"><table role=\"presentation\" width=\"480\" cellpadding=\"0\" cellspacing=\"0\" style=\"max-width:480px;width:100%;background-color:#ffffff;border-radius:12px;overflow:hidden;\"><tr><td style=\"background:linear-gradient(160deg,#0F6E8C 0%,#0A3040 100%);padding:28px 32px;text-align:center;\"><img src=\"{{logoUrl}}\" width=\"28\" height=\"28\" alt=\"SpeedClaim\" style=\"vertical-align:middle;margin-right:8px;border-radius:6px;\" /><span style=\"font-size:20px;font-weight:700;color:#ffffff;letter-spacing:-0.3px;vertical-align:middle;\">SpeedClaim</span></td></tr><tr><td style=\"padding:32px;\"><span style=\"display:inline-block;margin:0 0 12px;padding:4px 10px;background:#FEF6E6;color:#D9920A;border-radius:20px;font-size:11px;font-weight:700;\">Proposal Rejected</span><h1 style=\"margin:0 0 16px;font-size:22px;font-weight:600;color:#1A2230;\">Your proposal could not be approved</h1><p style=\"margin:0 0 16px;font-size:15px;line-height:1.6;color:#3C4654;\">Dear {{firstName}}, after careful review, your proposal <strong>{{proposalNumber}}</strong> has not been approved.</p><div style=\"margin:0 0 20px;padding:12px 16px;background:#FEF6E6;border-left:3px solid #D9920A;border-radius:4px;\"><p style=\"margin:0 0 4px;font-size:12px;font-weight:700;color:#D9920A;\">Reason</p><p style=\"margin:0;font-size:14px;color:#3C4654;\">{{rejectionReason}}</p></div><p style=\"margin:0;font-size:13px;line-height:1.5;color:#6B7685;\">You may contact your assigned agent or our support team for more information.</p></td></tr><tr><td style=\"padding:20px 32px;border-top:1px solid #E2E6EB;text-align:center;\"><p style=\"margin:0;font-size:12px;color:#6B7685;\">&copy; {{year}} SpeedClaim. All rights reserved.</p></td></tr></table></td></tr></table></body></html>",
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 6, 7, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            IsActive = true,
                            Subject = "Your proposal {{proposalNumber}} could not be approved",
                            TemplateKey = "ProposalRejected"
                        },
                        new
                        {
                            Id = new Guid("dd000008-0000-0000-0000-000000000008"),
                            BodyHtml = "<!DOCTYPE html><html lang=\"en\"><head><meta charset=\"UTF-8\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\"><title>Claim Approved</title></head><body style=\"margin:0;padding:0;background-color:#F7F9FA;font-family:Arial,'Helvetica Neue',Helvetica,sans-serif;\"><table role=\"presentation\" width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" style=\"background-color:#F7F9FA;padding:32px 16px;\"><tr><td align=\"center\"><table role=\"presentation\" width=\"480\" cellpadding=\"0\" cellspacing=\"0\" style=\"max-width:480px;width:100%;background-color:#ffffff;border-radius:12px;overflow:hidden;\"><tr><td style=\"background:linear-gradient(160deg,#0F6E8C 0%,#0A3040 100%);padding:28px 32px;text-align:center;\"><img src=\"{{logoUrl}}\" width=\"28\" height=\"28\" alt=\"SpeedClaim\" style=\"vertical-align:middle;margin-right:8px;border-radius:6px;\" /><span style=\"font-size:20px;font-weight:700;color:#ffffff;letter-spacing:-0.3px;vertical-align:middle;\">SpeedClaim</span></td></tr><tr><td style=\"padding:32px;\"><span style=\"display:inline-block;margin:0 0 12px;padding:4px 10px;background:#E8F7F1;color:#1F9D6B;border-radius:20px;font-size:11px;font-weight:700;\">Claim Approved</span><h1 style=\"margin:0 0 16px;font-size:22px;font-weight:600;color:#1A2230;\">Your claim has been approved</h1><p style=\"margin:0;font-size:15px;line-height:1.6;color:#3C4654;\">Dear {{firstName}}, your claim <strong>{{claimNumber}}</strong> has been approved following our investigation. Your payout will be processed shortly. You will receive a separate notification once the payment has been disbursed.</p></td></tr><tr><td style=\"padding:20px 32px;border-top:1px solid #E2E6EB;text-align:center;\"><p style=\"margin:0;font-size:12px;color:#6B7685;\">&copy; {{year}} SpeedClaim. All rights reserved.</p></td></tr></table></td></tr></table></body></html>",
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 6, 7, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            IsActive = true,
                            Subject = "Your claim {{claimNumber}} has been approved",
                            TemplateKey = "ClaimApproved"
                        },
                        new
                        {
                            Id = new Guid("dd000009-0000-0000-0000-000000000009"),
                            BodyHtml = "<!DOCTYPE html><html lang=\"en\"><head><meta charset=\"UTF-8\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\"><title>Claim Rejected</title></head><body style=\"margin:0;padding:0;background-color:#F7F9FA;font-family:Arial,'Helvetica Neue',Helvetica,sans-serif;\"><table role=\"presentation\" width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" style=\"background-color:#F7F9FA;padding:32px 16px;\"><tr><td align=\"center\"><table role=\"presentation\" width=\"480\" cellpadding=\"0\" cellspacing=\"0\" style=\"max-width:480px;width:100%;background-color:#ffffff;border-radius:12px;overflow:hidden;\"><tr><td style=\"background:linear-gradient(160deg,#0F6E8C 0%,#0A3040 100%);padding:28px 32px;text-align:center;\"><img src=\"{{logoUrl}}\" width=\"28\" height=\"28\" alt=\"SpeedClaim\" style=\"vertical-align:middle;margin-right:8px;border-radius:6px;\" /><span style=\"font-size:20px;font-weight:700;color:#ffffff;letter-spacing:-0.3px;vertical-align:middle;\">SpeedClaim</span></td></tr><tr><td style=\"padding:32px;\"><span style=\"display:inline-block;margin:0 0 12px;padding:4px 10px;background:#FBE9E9;color:#D14343;border-radius:20px;font-size:11px;font-weight:700;\">Claim Rejected</span><h1 style=\"margin:0 0 16px;font-size:22px;font-weight:600;color:#1A2230;\">Your claim could not be approved</h1><p style=\"margin:0 0 16px;font-size:15px;line-height:1.6;color:#3C4654;\">Dear {{firstName}}, after investigation, your claim <strong>{{claimNumber}}</strong> has not been approved.</p><div style=\"margin:0 0 20px;padding:12px 16px;background:#FBE9E9;border-left:3px solid #D14343;border-radius:4px;\"><p style=\"margin:0 0 4px;font-size:12px;font-weight:700;color:#D14343;\">Reason</p><p style=\"margin:0;font-size:14px;color:#3C4654;\">{{rejectionReason}}</p></div><p style=\"margin:0;font-size:13px;line-height:1.5;color:#6B7685;\">You may contact us to appeal this decision or submit additional evidence.</p></td></tr><tr><td style=\"padding:20px 32px;border-top:1px solid #E2E6EB;text-align:center;\"><p style=\"margin:0;font-size:12px;color:#6B7685;\">&copy; {{year}} SpeedClaim. All rights reserved.</p></td></tr></table></td></tr></table></body></html>",
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 6, 7, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            IsActive = true,
                            Subject = "Your claim {{claimNumber}} could not be approved",
                            TemplateKey = "ClaimRejected"
                        },
                        new
                        {
                            Id = new Guid("dd000010-0000-0000-0000-000000000010"),
                            BodyHtml = "<!DOCTYPE html><html lang=\"en\"><head><meta charset=\"UTF-8\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\"><title>Claim Settled</title></head><body style=\"margin:0;padding:0;background-color:#F7F9FA;font-family:Arial,'Helvetica Neue',Helvetica,sans-serif;\"><table role=\"presentation\" width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" style=\"background-color:#F7F9FA;padding:32px 16px;\"><tr><td align=\"center\"><table role=\"presentation\" width=\"480\" cellpadding=\"0\" cellspacing=\"0\" style=\"max-width:480px;width:100%;background-color:#ffffff;border-radius:12px;overflow:hidden;\"><tr><td style=\"background:linear-gradient(160deg,#0F6E8C 0%,#0A3040 100%);padding:28px 32px;text-align:center;\"><img src=\"{{logoUrl}}\" width=\"28\" height=\"28\" alt=\"SpeedClaim\" style=\"vertical-align:middle;margin-right:8px;border-radius:6px;\" /><span style=\"font-size:20px;font-weight:700;color:#ffffff;letter-spacing:-0.3px;vertical-align:middle;\">SpeedClaim</span></td></tr><tr><td style=\"padding:32px;\"><span style=\"display:inline-block;margin:0 0 12px;padding:4px 10px;background:#E8F7F1;color:#1F9D6B;border-radius:20px;font-size:11px;font-weight:700;\">Claim Settled</span><h1 style=\"margin:0 0 16px;font-size:22px;font-weight:600;color:#1A2230;\">Your claim payout has been processed</h1><p style=\"margin:0 0 20px;font-size:15px;line-height:1.6;color:#3C4654;\">Dear {{firstName}}, your claim <strong>{{claimNumber}}</strong> has been financially settled.</p><table role=\"presentation\" width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" style=\"border-collapse:collapse;border:1px solid #E2E6EB;border-radius:8px;overflow:hidden;\"><tr><td style=\"padding:12px 16px;background:#F7F9FA;font-size:12px;color:#6B7685;width:40%;\">Claim number</td><td style=\"padding:12px 16px;font-size:14px;font-weight:700;color:#1A2230;\">{{claimNumber}}</td></tr><tr><td style=\"padding:12px 16px;background:#F7F9FA;font-size:12px;color:#6B7685;\">Payout amount</td><td style=\"padding:12px 16px;font-size:14px;font-weight:700;color:#1F9D6B;\">{{payoutAmount}} INR</td></tr></table></td></tr><tr><td style=\"padding:20px 32px;border-top:1px solid #E2E6EB;text-align:center;\"><p style=\"margin:0;font-size:12px;color:#6B7685;\">&copy; {{year}} SpeedClaim. All rights reserved.</p></td></tr></table></td></tr></table></body></html>",
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 6, 7, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            IsActive = true,
                            Subject = "Your claim {{claimNumber}} has been settled",
                            TemplateKey = "ClaimSettled"
                        },
                        new
                        {
                            Id = new Guid("dd000011-0000-0000-0000-000000000011"),
                            BodyHtml = "<!DOCTYPE html><html lang=\"en\"><head><meta charset=\"UTF-8\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\"><title>Policy Cancelled</title></head><body style=\"margin:0;padding:0;background-color:#F7F9FA;font-family:Arial,'Helvetica Neue',Helvetica,sans-serif;\"><table role=\"presentation\" width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" style=\"background-color:#F7F9FA;padding:32px 16px;\"><tr><td align=\"center\"><table role=\"presentation\" width=\"480\" cellpadding=\"0\" cellspacing=\"0\" style=\"max-width:480px;width:100%;background-color:#ffffff;border-radius:12px;overflow:hidden;\"><tr><td style=\"background:linear-gradient(160deg,#0F6E8C 0%,#0A3040 100%);padding:28px 32px;text-align:center;\"><img src=\"{{logoUrl}}\" width=\"28\" height=\"28\" alt=\"SpeedClaim\" style=\"vertical-align:middle;margin-right:8px;border-radius:6px;\" /><span style=\"font-size:20px;font-weight:700;color:#ffffff;letter-spacing:-0.3px;vertical-align:middle;\">SpeedClaim</span></td></tr><tr><td style=\"padding:32px;\"><span style=\"display:inline-block;margin:0 0 12px;padding:4px 10px;background:#FEF6E6;color:#D9920A;border-radius:20px;font-size:11px;font-weight:700;\">Policy Cancelled</span><h1 style=\"margin:0 0 16px;font-size:22px;font-weight:600;color:#1A2230;\">Your policy has been cancelled</h1><p style=\"margin:0 0 16px;font-size:15px;line-height:1.6;color:#3C4654;\">Dear {{firstName}}, your policy <strong>{{policyNumber}}</strong> has been cancelled as requested.</p><p style=\"margin:0;font-size:13px;line-height:1.5;color:#6B7685;\">If you did not request this cancellation, please contact SpeedClaim support immediately.</p></td></tr><tr><td style=\"padding:20px 32px;border-top:1px solid #E2E6EB;text-align:center;\"><p style=\"margin:0;font-size:12px;color:#6B7685;\">&copy; {{year}} SpeedClaim. All rights reserved.</p></td></tr></table></td></tr></table></body></html>",
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 6, 7, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            IsActive = true,
                            Subject = "Your policy {{policyNumber}} has been cancelled",
                            TemplateKey = "PolicyCancelled"
                        },
                        new
                        {
                            Id = new Guid("dd000012-0000-0000-0000-000000000012"),
                            BodyHtml = "<!DOCTYPE html><html lang=\"en\"><head><meta charset=\"UTF-8\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\"><title>Claim Filed</title></head><body style=\"margin:0;padding:0;background-color:#F7F9FA;font-family:Arial,'Helvetica Neue',Helvetica,sans-serif;\"><table role=\"presentation\" width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" style=\"background-color:#F7F9FA;padding:32px 16px;\"><tr><td align=\"center\"><table role=\"presentation\" width=\"480\" cellpadding=\"0\" cellspacing=\"0\" style=\"max-width:480px;width:100%;background-color:#ffffff;border-radius:12px;overflow:hidden;\"><tr><td style=\"background:linear-gradient(160deg,#0F6E8C 0%,#0A3040 100%);padding:28px 32px;text-align:center;\"><img src=\"{{logoUrl}}\" width=\"28\" height=\"28\" alt=\"SpeedClaim\" style=\"vertical-align:middle;margin-right:8px;border-radius:6px;\" /><span style=\"font-size:20px;font-weight:700;color:#ffffff;letter-spacing:-0.3px;vertical-align:middle;\">SpeedClaim</span></td></tr><tr><td style=\"padding:32px;\"><span style=\"display:inline-block;margin:0 0 12px;padding:4px 10px;background:#E6F4F8;color:#0F6E8C;border-radius:20px;font-size:11px;font-weight:700;\">Claim Received</span><h1 style=\"margin:0 0 16px;font-size:22px;font-weight:600;color:#1A2230;\">Your claim has been filed</h1><p style=\"margin:0 0 20px;font-size:15px;line-height:1.6;color:#3C4654;\">Dear {{firstName}}, your claim has been successfully filed and assigned to our team for review.</p><table role=\"presentation\" width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" style=\"border-collapse:collapse;border:1px solid #E2E6EB;border-radius:8px;overflow:hidden;\"><tr><td style=\"padding:12px 16px;background:#F7F9FA;font-size:12px;color:#6B7685;width:40%;\">Claim number</td><td style=\"padding:12px 16px;font-size:14px;font-weight:700;color:#1A2230;\">{{claimNumber}}</td></tr><tr><td style=\"padding:12px 16px;background:#F7F9FA;font-size:12px;color:#6B7685;\">Policy number</td><td style=\"padding:12px 16px;font-size:14px;color:#1A2230;\">{{policyNumber}}</td></tr></table><p style=\"margin:20px 0 0;font-size:13px;line-height:1.5;color:#6B7685;\">You will be notified as the status of your claim is updated. You can track your claim in real time via the customer portal.</p></td></tr><tr><td style=\"padding:20px 32px;border-top:1px solid #E2E6EB;text-align:center;\"><p style=\"margin:0;font-size:12px;color:#6B7685;\">&copy; {{year}} SpeedClaim. All rights reserved.</p></td></tr></table></td></tr></table></body></html>",
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 6, 7, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            IsActive = true,
                            Subject = "Claim {{claimNumber}} filed — we have received your request",
                            TemplateKey = "ClaimIntimated"
                        },
                        new
                        {
                            Id = new Guid("dd000013-0000-0000-0000-000000000013"),
                            BodyHtml = "<!DOCTYPE html><html lang=\"en\"><head><meta charset=\"UTF-8\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\"><title>Endorsement Approved</title></head><body style=\"margin:0;padding:0;background-color:#F7F9FA;font-family:Arial,'Helvetica Neue',Helvetica,sans-serif;\"><table role=\"presentation\" width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" style=\"background-color:#F7F9FA;padding:32px 16px;\"><tr><td align=\"center\"><table role=\"presentation\" width=\"480\" cellpadding=\"0\" cellspacing=\"0\" style=\"max-width:480px;width:100%;background-color:#ffffff;border-radius:12px;overflow:hidden;\"><tr><td style=\"background:linear-gradient(160deg,#0F6E8C 0%,#0A3040 100%);padding:28px 32px;text-align:center;\"><img src=\"{{logoUrl}}\" width=\"28\" height=\"28\" alt=\"SpeedClaim\" style=\"vertical-align:middle;margin-right:8px;border-radius:6px;\" /><span style=\"font-size:20px;font-weight:700;color:#ffffff;letter-spacing:-0.3px;vertical-align:middle;\">SpeedClaim</span></td></tr><tr><td style=\"padding:32px;\"><span style=\"display:inline-block;margin:0 0 12px;padding:4px 10px;background:#E8F7F1;color:#1F9D6B;border-radius:20px;font-size:11px;font-weight:700;\">Endorsement Approved</span><h1 style=\"margin:0 0 16px;font-size:22px;font-weight:600;color:#1A2230;\">Your policy change has been approved</h1><p style=\"margin:0 0 20px;font-size:15px;line-height:1.6;color:#3C4654;\">Dear {{firstName}}, your endorsement request of type <strong>{{endorsementType}}</strong> for policy <strong>{{policyNumber}}</strong> has been approved and applied.</p><p style=\"margin:0;font-size:13px;line-height:1.5;color:#6B7685;\">The changes are now effective on your policy. Please log in to review the updated policy details.</p></td></tr><tr><td style=\"padding:20px 32px;border-top:1px solid #E2E6EB;text-align:center;\"><p style=\"margin:0;font-size:12px;color:#6B7685;\">&copy; {{year}} SpeedClaim. All rights reserved.</p></td></tr></table></td></tr></table></body></html>",
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 6, 7, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            IsActive = true,
                            Subject = "Endorsement approved for policy {{policyNumber}}",
                            TemplateKey = "EndorsementApproved"
                        },
                        new
                        {
                            Id = new Guid("dd000014-0000-0000-0000-000000000014"),
                            BodyHtml = "<!DOCTYPE html><html lang=\"en\"><head><meta charset=\"UTF-8\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\"><title>Endorsement Rejected</title></head><body style=\"margin:0;padding:0;background-color:#F7F9FA;font-family:Arial,'Helvetica Neue',Helvetica,sans-serif;\"><table role=\"presentation\" width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" style=\"background-color:#F7F9FA;padding:32px 16px;\"><tr><td align=\"center\"><table role=\"presentation\" width=\"480\" cellpadding=\"0\" cellspacing=\"0\" style=\"max-width:480px;width:100%;background-color:#ffffff;border-radius:12px;overflow:hidden;\"><tr><td style=\"background:linear-gradient(160deg,#0F6E8C 0%,#0A3040 100%);padding:28px 32px;text-align:center;\"><img src=\"{{logoUrl}}\" width=\"28\" height=\"28\" alt=\"SpeedClaim\" style=\"vertical-align:middle;margin-right:8px;border-radius:6px;\" /><span style=\"font-size:20px;font-weight:700;color:#ffffff;letter-spacing:-0.3px;vertical-align:middle;\">SpeedClaim</span></td></tr><tr><td style=\"padding:32px;\"><span style=\"display:inline-block;margin:0 0 12px;padding:4px 10px;background:#FBE9E9;color:#D14343;border-radius:20px;font-size:11px;font-weight:700;\">Endorsement Rejected</span><h1 style=\"margin:0 0 16px;font-size:22px;font-weight:600;color:#1A2230;\">Your policy change could not be approved</h1><p style=\"margin:0 0 16px;font-size:15px;line-height:1.6;color:#3C4654;\">Dear {{firstName}}, your endorsement request of type <strong>{{endorsementType}}</strong> for policy <strong>{{policyNumber}}</strong> has not been approved.</p><div style=\"margin:0 0 20px;padding:12px 16px;background:#FBE9E9;border-left:3px solid #D14343;border-radius:4px;\"><p style=\"margin:0 0 4px;font-size:12px;font-weight:700;color:#D14343;\">Reason</p><p style=\"margin:0;font-size:14px;color:#3C4654;\">{{rejectionReason}}</p></div><p style=\"margin:0;font-size:13px;line-height:1.5;color:#6B7685;\">Please contact your agent or our support team for further assistance.</p></td></tr><tr><td style=\"padding:20px 32px;border-top:1px solid #E2E6EB;text-align:center;\"><p style=\"margin:0;font-size:12px;color:#6B7685;\">&copy; {{year}} SpeedClaim. All rights reserved.</p></td></tr></table></td></tr></table></body></html>",
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 6, 7, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            IsActive = true,
                            Subject = "Endorsement request for policy {{policyNumber}} could not be approved",
                            TemplateKey = "EndorsementRejected"
                        },
                        new
                        {
                            Id = new Guid("dd000015-0000-0000-0000-000000000015"),
                            BodyHtml = "<!DOCTYPE html><html lang=\"en\"><head><meta charset=\"UTF-8\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\"><title>Grievance Filed</title></head><body style=\"margin:0;padding:0;background-color:#F7F9FA;font-family:Arial,'Helvetica Neue',Helvetica,sans-serif;\"><table role=\"presentation\" width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" style=\"background-color:#F7F9FA;padding:32px 16px;\"><tr><td align=\"center\"><table role=\"presentation\" width=\"480\" cellpadding=\"0\" cellspacing=\"0\" style=\"max-width:480px;width:100%;background-color:#ffffff;border-radius:12px;overflow:hidden;\"><tr><td style=\"background:linear-gradient(160deg,#0F6E8C 0%,#0A3040 100%);padding:28px 32px;text-align:center;\"><img src=\"{{logoUrl}}\" width=\"28\" height=\"28\" alt=\"SpeedClaim\" style=\"vertical-align:middle;margin-right:8px;border-radius:6px;\" /><span style=\"font-size:20px;font-weight:700;color:#ffffff;letter-spacing:-0.3px;vertical-align:middle;\">SpeedClaim</span></td></tr><tr><td style=\"padding:32px;\"><span style=\"display:inline-block;margin:0 0 12px;padding:4px 10px;background:#E6F4F8;color:#0F6E8C;border-radius:20px;font-size:11px;font-weight:700;\">Grievance Received</span><h1 style=\"margin:0 0 16px;font-size:22px;font-weight:600;color:#1A2230;\">Your grievance has been registered</h1><p style=\"margin:0 0 20px;font-size:15px;line-height:1.6;color:#3C4654;\">Dear {{firstName}}, we have received your grievance and assigned it reference number <strong>{{grievanceNumber}}</strong>. Our team will review it and get back to you within 7 business days.</p><p style=\"margin:0;font-size:13px;line-height:1.5;color:#6B7685;\">You can track the status of your grievance at any time via the customer portal.</p></td></tr><tr><td style=\"padding:20px 32px;border-top:1px solid #E2E6EB;text-align:center;\"><p style=\"margin:0;font-size:12px;color:#6B7685;\">&copy; {{year}} SpeedClaim. All rights reserved.</p></td></tr></table></td></tr></table></body></html>",
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 6, 7, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            IsActive = true,
                            Subject = "Grievance {{grievanceNumber}} received — we will get back to you",
                            TemplateKey = "GrievanceFiled"
                        },
                        new
                        {
                            Id = new Guid("dd000016-0000-0000-0000-000000000016"),
                            BodyHtml = "<!DOCTYPE html><html lang=\"en\"><head><meta charset=\"UTF-8\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\"><title>Grievance Resolved</title></head><body style=\"margin:0;padding:0;background-color:#F7F9FA;font-family:Arial,'Helvetica Neue',Helvetica,sans-serif;\"><table role=\"presentation\" width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" style=\"background-color:#F7F9FA;padding:32px 16px;\"><tr><td align=\"center\"><table role=\"presentation\" width=\"480\" cellpadding=\"0\" cellspacing=\"0\" style=\"max-width:480px;width:100%;background-color:#ffffff;border-radius:12px;overflow:hidden;\"><tr><td style=\"background:linear-gradient(160deg,#0F6E8C 0%,#0A3040 100%);padding:28px 32px;text-align:center;\"><img src=\"{{logoUrl}}\" width=\"28\" height=\"28\" alt=\"SpeedClaim\" style=\"vertical-align:middle;margin-right:8px;border-radius:6px;\" /><span style=\"font-size:20px;font-weight:700;color:#ffffff;letter-spacing:-0.3px;vertical-align:middle;\">SpeedClaim</span></td></tr><tr><td style=\"padding:32px;\"><span style=\"display:inline-block;margin:0 0 12px;padding:4px 10px;background:#E8F7F1;color:#1F9D6B;border-radius:20px;font-size:11px;font-weight:700;\">Grievance Resolved</span><h1 style=\"margin:0 0 16px;font-size:22px;font-weight:600;color:#1A2230;\">Your grievance has been resolved</h1><p style=\"margin:0 0 20px;font-size:15px;line-height:1.6;color:#3C4654;\">Dear {{firstName}}, your grievance <strong>{{grievanceNumber}}</strong> has been reviewed and resolved by our team.</p><div style=\"margin:0 0 20px;padding:12px 16px;background:#F7F9FA;border-left:3px solid #0F6E8C;border-radius:4px;\"><p style=\"margin:0 0 4px;font-size:12px;font-weight:700;color:#0F6E8C;\">Resolution</p><p style=\"margin:0;font-size:14px;color:#3C4654;\">{{resolutionNotes}}</p></div><p style=\"margin:0;font-size:13px;line-height:1.5;color:#6B7685;\">If you are not satisfied with the resolution, you may escalate via the customer portal.</p></td></tr><tr><td style=\"padding:20px 32px;border-top:1px solid #E2E6EB;text-align:center;\"><p style=\"margin:0;font-size:12px;color:#6B7685;\">&copy; {{year}} SpeedClaim. All rights reserved.</p></td></tr></table></td></tr></table></body></html>",
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 6, 7, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            IsActive = true,
                            Subject = "Your grievance {{grievanceNumber}} has been resolved",
                            TemplateKey = "GrievanceResolved"
                        },
                        new
                        {
                            Id = new Guid("dd000017-0000-0000-0000-000000000017"),
                            BodyHtml = "<!DOCTYPE html><html lang=\"en\"><head><meta charset=\"UTF-8\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\"><title>Premium Overdue</title></head><body style=\"margin:0;padding:0;background-color:#F7F9FA;font-family:Arial,'Helvetica Neue',Helvetica,sans-serif;\"><table role=\"presentation\" width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" style=\"background-color:#F7F9FA;padding:32px 16px;\"><tr><td align=\"center\"><table role=\"presentation\" width=\"480\" cellpadding=\"0\" cellspacing=\"0\" style=\"max-width:480px;width:100%;background-color:#ffffff;border-radius:12px;overflow:hidden;\"><tr><td style=\"background:linear-gradient(160deg,#0F6E8C 0%,#0A3040 100%);padding:28px 32px;text-align:center;\"><img src=\"{{logoUrl}}\" width=\"28\" height=\"28\" alt=\"SpeedClaim\" style=\"vertical-align:middle;margin-right:8px;border-radius:6px;\" /><span style=\"font-size:20px;font-weight:700;color:#ffffff;letter-spacing:-0.3px;vertical-align:middle;\">SpeedClaim</span></td></tr><tr><td style=\"padding:32px;\"><span style=\"display:inline-block;margin:0 0 12px;padding:4px 10px;background:#FBE9E9;color:#D14343;border-radius:20px;font-size:11px;font-weight:700;\">Premium Overdue</span><h1 style=\"margin:0 0 16px;font-size:22px;font-weight:600;color:#1A2230;\">Your premium payment is overdue</h1><p style=\"margin:0 0 20px;font-size:15px;line-height:1.6;color:#3C4654;\">Dear {{firstName}}, a premium payment for your policy is now overdue. Please make your payment as soon as possible to avoid a lapse in coverage.</p><table role=\"presentation\" width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" style=\"border-collapse:collapse;border:1px solid #E2E6EB;border-radius:8px;overflow:hidden;\"><tr><td style=\"padding:12px 16px;background:#F7F9FA;font-size:12px;color:#6B7685;width:40%;\">Policy number</td><td style=\"padding:12px 16px;font-size:14px;font-weight:700;color:#1A2230;\">{{policyNumber}}</td></tr><tr><td style=\"padding:12px 16px;background:#F7F9FA;font-size:12px;color:#6B7685;\">Amount due</td><td style=\"padding:12px 16px;font-size:14px;font-weight:700;color:#D14343;\">{{amount}} INR</td></tr><tr><td style=\"padding:12px 16px;background:#F7F9FA;font-size:12px;color:#6B7685;\">Was due on</td><td style=\"padding:12px 16px;font-size:14px;color:#1A2230;\">{{dueDate}}</td></tr></table><p style=\"margin:20px 0 0;font-size:13px;line-height:1.5;color:#6B7685;\">Log in to the customer portal to make your payment immediately.</p></td></tr><tr><td style=\"padding:20px 32px;border-top:1px solid #E2E6EB;text-align:center;\"><p style=\"margin:0;font-size:12px;color:#6B7685;\">&copy; {{year}} SpeedClaim. All rights reserved.</p></td></tr></table></td></tr></table></body></html>",
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 6, 7, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            IsActive = true,
                            Subject = "Action required — premium overdue for policy {{policyNumber}}",
                            TemplateKey = "PremiumOverdue"
                        });
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.Endorsement", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("description");

                    b.Property<string>("EndorsementType")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("endorsement_type");

                    b.Property<string>("NewValue")
                        .HasColumnType("text")
                        .HasColumnName("new_value");

                    b.Property<string>("OldValue")
                        .HasColumnType("text")
                        .HasColumnName("old_value");

                    b.Property<Guid>("PolicyId")
                        .HasColumnType("uuid")
                        .HasColumnName("policy_id");

                    b.Property<Guid>("RequestedById")
                        .HasColumnType("uuid")
                        .HasColumnName("requested_by_id");

                    b.Property<DateTimeOffset?>("ReviewedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("reviewed_at");

                    b.Property<Guid?>("ReviewedById")
                        .HasColumnType("uuid")
                        .HasColumnName("reviewed_by_id");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("status");

                    b.Property<DateTimeOffset?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.HasKey("Id")
                        .HasName("PK_endorsements");

                    b.HasIndex("PolicyId");

                    b.HasIndex("RequestedById");

                    b.HasIndex("ReviewedById");

                    b.ToTable("endorsements");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.ExternalIdentity", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTimeOffset?>("LastAuthenticatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("last_authenticated_at");

                    b.Property<DateTimeOffset>("LinkedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("linked_at");

                    b.Property<string>("Provider")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("provider");

                    b.Property<string>("Subject")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)")
                        .HasColumnName("subject");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid")
                        .HasColumnName("user_id");

                    b.HasKey("Id")
                        .HasName("PK_external_identities");

                    b.HasIndex("Provider", "Subject")
                        .IsUnique()
                        .HasDatabaseName("uq_external_identities_provider_subject");

                    b.HasIndex("UserId", "Provider")
                        .IsUnique()
                        .HasDatabaseName("uq_external_identities_user_provider");

                    b.ToTable("external_identities");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.Grievance", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid?>("AssignedToId")
                        .HasColumnType("uuid")
                        .HasColumnName("assigned_to_id");

                    b.Property<string>("AttachmentPath")
                        .HasColumnType("text")
                        .HasColumnName("attachment_path");

                    b.Property<string>("Category")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("category");

                    b.Property<Guid?>("ClaimId")
                        .HasColumnType("uuid")
                        .HasColumnName("claim_id");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<Guid>("CustomerId")
                        .HasColumnType("uuid")
                        .HasColumnName("customer_id");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("description");

                    b.Property<string>("GrievanceNumber")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("character varying(30)")
                        .HasColumnName("grievance_number");

                    b.Property<Guid?>("PolicyId")
                        .HasColumnType("uuid")
                        .HasColumnName("policy_id");

                    b.Property<string>("ResolutionNotes")
                        .HasColumnType("text")
                        .HasColumnName("resolution_notes");

                    b.Property<DateTimeOffset?>("ResolvedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("resolved_at");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("status");

                    b.Property<DateTimeOffset?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.HasKey("Id")
                        .HasName("PK_grievances");

                    b.HasIndex("AssignedToId");

                    b.HasIndex("ClaimId");

                    b.HasIndex("CustomerId");

                    b.HasIndex("GrievanceNumber")
                        .IsUnique()
                        .HasDatabaseName("uq_grievances_grievance_number");

                    b.HasIndex("PolicyId");

                    b.ToTable("grievances");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.HealthClaimDetail", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTime>("AdmissionDate")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("admission_date");

                    b.Property<Guid>("ClaimId")
                        .HasColumnType("uuid")
                        .HasColumnName("claim_id");

                    b.Property<string>("Diagnosis")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("diagnosis");

                    b.Property<DateTime?>("DischargeDate")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("discharge_date");

                    b.Property<string>("HospitalAddress")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("hospital_address");

                    b.Property<string>("HospitalName")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("character varying(150)")
                        .HasColumnName("hospital_name");

                    b.Property<DateTimeOffset?>("PreAuthApprovedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("pre_auth_approved_at");

                    b.Property<DateTimeOffset?>("PreAuthRequestedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("pre_auth_requested_at");

                    b.Property<string>("TpaReferenceNumber")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("tpa_reference_number");

                    b.Property<string>("TreatmentType")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("treatment_type");

                    b.HasKey("Id")
                        .HasName("PK_health_claim_details");

                    b.HasIndex("ClaimId")
                        .IsUnique();

                    b.ToTable("health_claim_details");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.HealthDetail", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<decimal>("CopayPercentage")
                        .HasColumnType("decimal(5,2)")
                        .HasColumnName("copay_percentage");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<bool>("MaternityCovered")
                        .HasColumnType("boolean")
                        .HasColumnName("maternity_covered");

                    b.Property<string>("NetworkHospitalCoverage")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("network_hospital_coverage");

                    b.Property<Guid?>("PolicyId")
                        .HasColumnType("uuid")
                        .HasColumnName("policy_id");

                    b.Property<string>("PreExistingConditions")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("pre_existing_conditions");

                    b.Property<Guid>("ProposalId")
                        .HasColumnType("uuid")
                        .HasColumnName("proposal_id");

                    b.Property<decimal>("RoomRentLimit")
                        .HasColumnType("decimal(10,2)")
                        .HasColumnName("room_rent_limit");

                    b.Property<string>("TpaName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("tpa_name");

                    b.Property<DateTimeOffset?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.HasKey("Id")
                        .HasName("PK_health_details");

                    b.HasIndex("PolicyId")
                        .IsUnique();

                    b.HasIndex("ProposalId")
                        .IsUnique();

                    b.ToTable("health_details");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.InsuranceProduct", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<bool>("AllowsFamilyFloater")
                        .HasColumnType("boolean")
                        .HasColumnName("allows_family_floater");

                    b.Property<string>("CoverageOptionsJson")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasColumnType("jsonb")
                        .HasDefaultValue("[]")
                        .HasColumnName("coverage_options_json");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<Guid?>("CreatedById")
                        .HasColumnType("uuid")
                        .HasColumnName("created_by_id");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("description");

                    b.Property<string>("Domain")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("domain");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean")
                        .HasColumnName("is_active");

                    b.Property<bool>("IsAvailableForSale")
                        .HasColumnType("boolean")
                        .HasColumnName("is_available_for_sale");

                    b.Property<int>("MaxAge")
                        .HasColumnType("integer")
                        .HasColumnName("max_age");

                    b.Property<int>("MaxFamilyMembers")
                        .HasColumnType("integer")
                        .HasColumnName("max_family_members");

                    b.Property<decimal>("MaxSumAssured")
                        .HasColumnType("decimal(15,2)")
                        .HasColumnName("max_sum_assured");

                    b.Property<int>("MaxTenureYears")
                        .HasColumnType("integer")
                        .HasColumnName("max_tenure_years");

                    b.Property<int>("MinAge")
                        .HasColumnType("integer")
                        .HasColumnName("min_age");

                    b.Property<decimal>("MinSumAssured")
                        .HasColumnType("decimal(15,2)")
                        .HasColumnName("min_sum_assured");

                    b.Property<int>("MinTenureYears")
                        .HasColumnType("integer")
                        .HasColumnName("min_tenure_years");

                    b.Property<string>("MotorVehicleType")
                        .HasColumnType("text")
                        .HasColumnName("motor_vehicle_type");

                    b.Property<string>("ProductName")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("character varying(150)")
                        .HasColumnName("product_name");

                    b.Property<decimal?>("SumAssuredIncrement")
                        .HasColumnType("decimal(15,2)")
                        .HasColumnName("sum_assured_increment");

                    b.Property<string>("Uin")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("uin");

                    b.Property<DateTimeOffset?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<int>("WaitingPeriodDays")
                        .HasColumnType("integer")
                        .HasColumnName("waiting_period_days");

                    b.HasKey("Id")
                        .HasName("PK_insurance_products");

                    b.HasIndex("CreatedById");

                    b.HasIndex("Uin")
                        .IsUnique()
                        .HasDatabaseName("uq_insurance_products_uin");

                    b.ToTable("insurance_products");

                    b.HasData(
                        new
                        {
                            Id = new Guid("71000000-0000-0000-0000-000000000001"),
                            AllowsFamilyFloater = true,
                            CoverageOptionsJson = "[300000,500000,1000000,1500000]",
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            Description = "Fictional family health cover for catalog, quotation, and policy-Q&A testing.",
                            Domain = "Health",
                            IsActive = true,
                            IsAvailableForSale = true,
                            MaxAge = 60,
                            MaxFamilyMembers = 6,
                            MaxSumAssured = 1500000m,
                            MaxTenureYears = 1,
                            MinAge = 18,
                            MinSumAssured = 300000m,
                            MinTenureYears = 1,
                            ProductName = "CareNest Family Shield",
                            Uin = "UIN-HC-DEMO-2026-01",
                            WaitingPeriodDays = 30
                        },
                        new
                        {
                            Id = new Guid("71000000-0000-0000-0000-000000000002"),
                            AllowsFamilyFloater = false,
                            CoverageOptionsJson = "[]",
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            Description = "Fictional pure-protection term cover for catalog, quotation, and policy-Q&A testing.",
                            Domain = "Life",
                            IsActive = true,
                            IsAvailableForSale = true,
                            MaxAge = 55,
                            MaxFamilyMembers = 1,
                            MaxSumAssured = 10000000m,
                            MaxTenureYears = 30,
                            MinAge = 18,
                            MinSumAssured = 2500000m,
                            MinTenureYears = 10,
                            ProductName = "Horizon Term Protect",
                            SumAssuredIncrement = 2500000m,
                            Uin = "UIN-LI-DEMO-2026-01",
                            WaitingPeriodDays = 0
                        },
                        new
                        {
                            Id = new Guid("71000000-0000-0000-0000-000000000003"),
                            AllowsFamilyFloater = false,
                            CoverageOptionsJson = "[]",
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            Description = "Fictional private-car comprehensive cover for catalog, quotation, and claims-flow testing.",
                            Domain = "Motor",
                            IsActive = true,
                            IsAvailableForSale = true,
                            MaxAge = 75,
                            MaxFamilyMembers = 1,
                            MaxSumAssured = 2000000m,
                            MaxTenureYears = 1,
                            MinAge = 18,
                            MinSumAssured = 100000m,
                            MinTenureYears = 1,
                            MotorVehicleType = "PrivateCar",
                            ProductName = "DriveSure Comprehensive",
                            Uin = "UIN-MO-DEMO-2026-01",
                            WaitingPeriodDays = 0
                        });
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.KycRecord", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("AadhaarDocumentKey")
                        .HasColumnType("text")
                        .HasColumnName("aadhaar_document_key");

                    b.Property<string>("AadhaarNumber")
                        .HasColumnType("text")
                        .HasColumnName("aadhaar_number");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("KycStatus")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("kyc_status");

                    b.Property<string>("PanDocumentKey")
                        .HasColumnType("text")
                        .HasColumnName("pan_document_key");

                    b.Property<string>("PanNumber")
                        .HasColumnType("text")
                        .HasColumnName("pan_number");

                    b.Property<string>("RejectionReason")
                        .HasColumnType("text")
                        .HasColumnName("rejection_reason");

                    b.Property<DateTimeOffset?>("ReviewedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("reviewed_at");

                    b.Property<Guid?>("ReviewedById")
                        .HasColumnType("uuid")
                        .HasColumnName("reviewed_by_id");

                    b.Property<DateTimeOffset?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid")
                        .HasColumnName("user_id");

                    b.HasKey("Id")
                        .HasName("PK_kyc_records");

                    b.HasIndex("ReviewedById");

                    b.HasIndex("UserId")
                        .IsUnique();

                    b.ToTable("kyc_records");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.LifeClaimDetail", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("CauseOfDeath")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("cause_of_death");

                    b.Property<string>("CertifyingDoctor")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("certifying_doctor");

                    b.Property<Guid>("ClaimId")
                        .HasColumnType("uuid")
                        .HasColumnName("claim_id");

                    b.Property<string>("ClaimReason")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("claim_reason");

                    b.Property<string>("ClaimantName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("claimant_name");

                    b.Property<string>("ClaimantRelationship")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("claimant_relationship");

                    b.Property<string>("DeathCertificateNumber")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("death_certificate_number");

                    b.Property<string>("PlaceOfDeath")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("place_of_death");

                    b.HasKey("Id")
                        .HasName("PK_life_claim_details");

                    b.HasIndex("ClaimId")
                        .IsUnique();

                    b.ToTable("life_claim_details");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.LifeDetail", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<decimal>("DeathBenefit")
                        .HasColumnType("decimal(15,2)")
                        .HasColumnName("death_benefit");

                    b.Property<bool>("LoanEligible")
                        .HasColumnType("boolean")
                        .HasColumnName("loan_eligible");

                    b.Property<decimal>("MaturityBenefit")
                        .HasColumnType("decimal(15,2)")
                        .HasColumnName("maturity_benefit");

                    b.Property<Guid?>("PolicyId")
                        .HasColumnType("uuid")
                        .HasColumnName("policy_id");

                    b.Property<string>("PolicySubtype")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("policy_subtype");

                    b.Property<Guid>("ProposalId")
                        .HasColumnType("uuid")
                        .HasColumnName("proposal_id");

                    b.Property<bool>("SurrenderValueApplicable")
                        .HasColumnType("boolean")
                        .HasColumnName("surrender_value_applicable");

                    b.Property<DateTimeOffset?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.HasKey("Id")
                        .HasName("PK_life_details");

                    b.HasIndex("PolicyId")
                        .IsUnique();

                    b.HasIndex("ProposalId")
                        .IsUnique();

                    b.ToTable("life_details");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.MotorClaimDetail", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("AccidentLocation")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("accident_location");

                    b.Property<Guid>("ClaimId")
                        .HasColumnType("uuid")
                        .HasColumnName("claim_id");

                    b.Property<decimal>("EstimatedRepairCost")
                        .HasColumnType("decimal(10,2)")
                        .HasColumnName("estimated_repair_cost");

                    b.Property<string>("FirNumber")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("fir_number");

                    b.Property<string>("GarageName")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("character varying(150)")
                        .HasColumnName("garage_name");

                    b.Property<DateTime?>("SurveyDate")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("survey_date");

                    b.Property<string>("SurveyorRemarks")
                        .HasColumnType("text")
                        .HasColumnName("surveyor_remarks");

                    b.HasKey("Id")
                        .HasName("PK_motor_claim_details");

                    b.HasIndex("ClaimId")
                        .IsUnique();

                    b.ToTable("motor_claim_details");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.MotorDetail", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("ChassisNumber")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("chassis_number");

                    b.Property<string>("CoverType")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("cover_type");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("EngineNumber")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("engine_number");

                    b.Property<decimal>("Idv")
                        .HasColumnType("decimal(10,2)")
                        .HasColumnName("idv");

                    b.Property<int>("ManufactureYear")
                        .HasColumnType("integer")
                        .HasColumnName("manufacture_year");

                    b.Property<Guid?>("PolicyId")
                        .HasColumnType("uuid")
                        .HasColumnName("policy_id");

                    b.Property<Guid>("ProposalId")
                        .HasColumnType("uuid")
                        .HasColumnName("proposal_id");

                    b.Property<DateTimeOffset?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<string>("VehicleMake")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("vehicle_make");

                    b.Property<string>("VehicleModel")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("vehicle_model");

                    b.Property<string>("VehicleNumber")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasColumnName("vehicle_number");

                    b.Property<string>("VehicleType")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("vehicle_type");

                    b.HasKey("Id")
                        .HasName("PK_motor_details");

                    b.HasIndex("PolicyId")
                        .IsUnique();

                    b.HasIndex("ProposalId")
                        .IsUnique();

                    b.ToTable("motor_details");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.Nominee", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("AppointeeName")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("appointee_name");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<DateOnly>("DateOfBirth")
                        .HasColumnType("date")
                        .HasColumnName("date_of_birth");

                    b.Property<string>("FullName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("full_name");

                    b.Property<bool>("IsMinor")
                        .HasColumnType("boolean")
                        .HasColumnName("is_minor");

                    b.Property<Guid?>("PolicyId")
                        .HasColumnType("uuid")
                        .HasColumnName("policy_id");

                    b.Property<Guid>("ProposalId")
                        .HasColumnType("uuid")
                        .HasColumnName("proposal_id");

                    b.Property<string>("Relationship")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("relationship");

                    b.Property<decimal>("SharePercentage")
                        .HasColumnType("decimal(5,2)")
                        .HasColumnName("share_percentage");

                    b.Property<DateTimeOffset?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.HasKey("Id")
                        .HasName("PK_nominees");

                    b.HasIndex("PolicyId");

                    b.HasIndex("ProposalId");

                    b.ToTable("nominees");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.Notification", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<bool>("IsRead")
                        .HasColumnType("boolean")
                        .HasColumnName("is_read");

                    b.Property<string>("Message")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("message");

                    b.Property<string>("RedirectUrl")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)")
                        .HasColumnName("redirect_url");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("title");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("type");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid")
                        .HasColumnName("user_id");

                    b.HasKey("Id")
                        .HasName("PK_notifications");

                    b.HasIndex("UserId");

                    b.ToTable("notifications");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.Policy", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid?>("AgentId")
                        .HasColumnType("uuid")
                        .HasColumnName("agent_id");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<Guid>("CustomerId")
                        .HasColumnType("uuid")
                        .HasColumnName("customer_id");

                    b.Property<DateTimeOffset?>("DeletedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("deleted_at");

                    b.Property<DateTime>("EndDate")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("end_date");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("boolean")
                        .HasColumnName("is_deleted");

                    b.Property<DateTimeOffset?>("IssuedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("issued_at");

                    b.Property<string>("PaymentFrequency")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("payment_frequency");

                    b.Property<string>("PolicyNumber")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("character varying(30)")
                        .HasColumnName("policy_number");

                    b.Property<string>("PolicyType")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("policy_type");

                    b.Property<decimal>("PremiumAmount")
                        .HasColumnType("decimal(10,2)")
                        .HasColumnName("premium_amount");

                    b.Property<Guid?>("ProductBrochureId")
                        .HasColumnType("uuid")
                        .HasColumnName("product_brochure_id");

                    b.Property<Guid>("ProductId")
                        .HasColumnType("uuid")
                        .HasColumnName("product_id");

                    b.Property<Guid?>("ProposalId")
                        .HasColumnType("uuid")
                        .HasColumnName("proposal_id");

                    b.Property<DateTime>("StartDate")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("start_date");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("status");

                    b.Property<decimal>("SumAssured")
                        .HasColumnType("decimal(15,2)")
                        .HasColumnName("sum_assured");

                    b.Property<DateTimeOffset?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.HasKey("Id")
                        .HasName("PK_policies");

                    b.HasIndex("AgentId");

                    b.HasIndex("CustomerId");

                    b.HasIndex("PolicyNumber")
                        .IsUnique()
                        .HasDatabaseName("uq_policies_policy_number");

                    b.HasIndex("ProductBrochureId");

                    b.HasIndex("ProductId");

                    b.HasIndex("ProposalId")
                        .IsUnique();

                    b.ToTable("policies");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.PolicyAssistantConversation", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid>("BrochureId")
                        .HasColumnType("uuid")
                        .HasColumnName("brochure_id");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<Guid>("CreatedByUserId")
                        .HasColumnType("uuid")
                        .HasColumnName("created_by_user_id");

                    b.Property<Guid>("PolicyId")
                        .HasColumnType("uuid")
                        .HasColumnName("policy_id");

                    b.Property<DateTimeOffset>("RetainUntil")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("retain_until");

                    b.Property<DateTimeOffset>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.HasKey("Id")
                        .HasName("PK_policy_assistant_conversations");

                    b.HasIndex("BrochureId");

                    b.HasIndex("CreatedByUserId");

                    b.HasIndex("RetainUntil")
                        .HasDatabaseName("ix_policy_assistant_conversations_retain_until");

                    b.HasIndex("PolicyId", "CreatedByUserId", "UpdatedAt")
                        .HasDatabaseName("ix_policy_assistant_conversations_policy_creator_updated");

                    b.ToTable("policy_assistant_conversations");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.PolicyAssistantMessage", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("CitationsJson")
                        .HasColumnType("text")
                        .HasColumnName("citations_json");

                    b.Property<string>("Content")
                        .IsRequired()
                        .HasMaxLength(4000)
                        .HasColumnType("character varying(4000)")
                        .HasColumnName("content");

                    b.Property<Guid>("ConversationId")
                        .HasColumnType("uuid")
                        .HasColumnName("conversation_id");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("EvidenceStatus")
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("evidence_status");

                    b.Property<string>("Model")
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)")
                        .HasColumnName("model");

                    b.Property<string>("PromptVersion")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("prompt_version");

                    b.Property<string>("Role")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("role");

                    b.HasKey("Id")
                        .HasName("PK_policy_assistant_messages");

                    b.HasIndex("ConversationId", "CreatedAt")
                        .HasDatabaseName("ix_policy_assistant_messages_conversation_created");

                    b.ToTable("policy_assistant_messages");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.PolicyMember", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<Guid>("CustomerMemberId")
                        .HasColumnType("uuid")
                        .HasColumnName("customer_member_id");

                    b.Property<Guid>("PolicyId")
                        .HasColumnType("uuid")
                        .HasColumnName("policy_id");

                    b.HasKey("Id")
                        .HasName("PK_policy_members");

                    b.HasIndex("CustomerMemberId");

                    b.HasIndex("PolicyId");

                    b.ToTable("policy_members");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.PolicyStatusHistory", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTimeOffset>("ChangedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("changed_at");

                    b.Property<Guid?>("ChangedById")
                        .HasColumnType("uuid")
                        .HasColumnName("changed_by_id");

                    b.Property<string>("NewStatus")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("new_status");

                    b.Property<string>("OldStatus")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("old_status");

                    b.Property<Guid>("PolicyId")
                        .HasColumnType("uuid")
                        .HasColumnName("policy_id");

                    b.Property<string>("Reason")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("reason");

                    b.HasKey("Id")
                        .HasName("PK_policy_status_history");

                    b.HasIndex("ChangedById");

                    b.HasIndex("PolicyId");

                    b.ToTable("policy_status_histories");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.PremiumPayment", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<decimal>("Amount")
                        .HasColumnType("decimal(10,2)")
                        .HasColumnName("amount");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("Currency")
                        .IsRequired()
                        .HasMaxLength(3)
                        .HasColumnType("character varying(3)")
                        .HasColumnName("currency");

                    b.Property<Guid>("CustomerId")
                        .HasColumnType("uuid")
                        .HasColumnName("customer_id");

                    b.Property<DateTimeOffset?>("PaidAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("paid_at");

                    b.Property<string>("PaymentMethod")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("payment_method");

                    b.Property<string>("PaymentType")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("payment_type");

                    b.Property<Guid?>("PolicyId")
                        .HasColumnType("uuid")
                        .HasColumnName("policy_id");

                    b.Property<Guid?>("ProposalId")
                        .HasColumnType("uuid")
                        .HasColumnName("proposal_id");

                    b.Property<string>("ReceiptUrl")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("receipt_url");

                    b.Property<Guid?>("ScheduleId")
                        .HasColumnType("uuid")
                        .HasColumnName("schedule_id");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("status");

                    b.Property<string>("StripeChargeId")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)")
                        .HasColumnName("stripe_charge_id");

                    b.Property<string>("StripePaymentIntentId")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)")
                        .HasColumnName("stripe_payment_intent_id");

                    b.Property<DateTimeOffset?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.HasKey("Id")
                        .HasName("PK_premium_payments");

                    b.HasIndex("CustomerId");

                    b.HasIndex("PolicyId");

                    b.HasIndex("ProposalId");

                    b.HasIndex("ScheduleId")
                        .IsUnique();

                    b.ToTable("premium_payments");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.PremiumRateTable", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<int>("AgeMax")
                        .HasColumnType("integer")
                        .HasColumnName("age_max");

                    b.Property<int>("AgeMin")
                        .HasColumnType("integer")
                        .HasColumnName("age_min");

                    b.Property<decimal>("AnnualPremium")
                        .HasColumnType("decimal(10,2)")
                        .HasColumnName("annual_premium");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<Guid>("ProductId")
                        .HasColumnType("uuid")
                        .HasColumnName("product_id");

                    b.Property<decimal>("SumAssuredMax")
                        .HasColumnType("decimal(15,2)")
                        .HasColumnName("sum_assured_max");

                    b.Property<decimal>("SumAssuredMin")
                        .HasColumnType("decimal(15,2)")
                        .HasColumnName("sum_assured_min");

                    b.HasKey("Id")
                        .HasName("PK_premium_rate_tables");

                    b.HasIndex("ProductId");

                    b.ToTable("premium_rate_tables");

                    b.HasData(
                        new
                        {
                            Id = new Guid("71100000-0000-0000-0000-000000000001"),
                            AgeMax = 30,
                            AgeMin = 18,
                            AnnualPremium = 4800m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            ProductId = new Guid("71000000-0000-0000-0000-000000000001"),
                            SumAssuredMax = 300000m,
                            SumAssuredMin = 300000m
                        },
                        new
                        {
                            Id = new Guid("71100000-0000-0000-0000-000000000002"),
                            AgeMax = 30,
                            AgeMin = 18,
                            AnnualPremium = 6800m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            ProductId = new Guid("71000000-0000-0000-0000-000000000001"),
                            SumAssuredMax = 500000m,
                            SumAssuredMin = 500000m
                        },
                        new
                        {
                            Id = new Guid("71100000-0000-0000-0000-000000000003"),
                            AgeMax = 30,
                            AgeMin = 18,
                            AnnualPremium = 9900m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            ProductId = new Guid("71000000-0000-0000-0000-000000000001"),
                            SumAssuredMax = 1000000m,
                            SumAssuredMin = 1000000m
                        },
                        new
                        {
                            Id = new Guid("71100000-0000-0000-0000-000000000004"),
                            AgeMax = 30,
                            AgeMin = 18,
                            AnnualPremium = 13400m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            ProductId = new Guid("71000000-0000-0000-0000-000000000001"),
                            SumAssuredMax = 1500000m,
                            SumAssuredMin = 1500000m
                        },
                        new
                        {
                            Id = new Guid("71100000-0000-0000-0000-000000000005"),
                            AgeMax = 40,
                            AgeMin = 31,
                            AnnualPremium = 5900m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            ProductId = new Guid("71000000-0000-0000-0000-000000000001"),
                            SumAssuredMax = 300000m,
                            SumAssuredMin = 300000m
                        },
                        new
                        {
                            Id = new Guid("71100000-0000-0000-0000-000000000006"),
                            AgeMax = 40,
                            AgeMin = 31,
                            AnnualPremium = 8300m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            ProductId = new Guid("71000000-0000-0000-0000-000000000001"),
                            SumAssuredMax = 500000m,
                            SumAssuredMin = 500000m
                        },
                        new
                        {
                            Id = new Guid("71100000-0000-0000-0000-000000000007"),
                            AgeMax = 40,
                            AgeMin = 31,
                            AnnualPremium = 12100m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            ProductId = new Guid("71000000-0000-0000-0000-000000000001"),
                            SumAssuredMax = 1000000m,
                            SumAssuredMin = 1000000m
                        },
                        new
                        {
                            Id = new Guid("71100000-0000-0000-0000-000000000008"),
                            AgeMax = 40,
                            AgeMin = 31,
                            AnnualPremium = 16700m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            ProductId = new Guid("71000000-0000-0000-0000-000000000001"),
                            SumAssuredMax = 1500000m,
                            SumAssuredMin = 1500000m
                        },
                        new
                        {
                            Id = new Guid("71100000-0000-0000-0000-000000000009"),
                            AgeMax = 50,
                            AgeMin = 41,
                            AnnualPremium = 8300m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            ProductId = new Guid("71000000-0000-0000-0000-000000000001"),
                            SumAssuredMax = 300000m,
                            SumAssuredMin = 300000m
                        },
                        new
                        {
                            Id = new Guid("71100000-0000-0000-0000-000000000010"),
                            AgeMax = 50,
                            AgeMin = 41,
                            AnnualPremium = 11900m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            ProductId = new Guid("71000000-0000-0000-0000-000000000001"),
                            SumAssuredMax = 500000m,
                            SumAssuredMin = 500000m
                        },
                        new
                        {
                            Id = new Guid("71100000-0000-0000-0000-000000000011"),
                            AgeMax = 50,
                            AgeMin = 41,
                            AnnualPremium = 17600m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            ProductId = new Guid("71000000-0000-0000-0000-000000000001"),
                            SumAssuredMax = 1000000m,
                            SumAssuredMin = 1000000m
                        },
                        new
                        {
                            Id = new Guid("71100000-0000-0000-0000-000000000012"),
                            AgeMax = 50,
                            AgeMin = 41,
                            AnnualPremium = 24300m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            ProductId = new Guid("71000000-0000-0000-0000-000000000001"),
                            SumAssuredMax = 1500000m,
                            SumAssuredMin = 1500000m
                        },
                        new
                        {
                            Id = new Guid("71100000-0000-0000-0000-000000000013"),
                            AgeMax = 60,
                            AgeMin = 51,
                            AnnualPremium = 12500m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            ProductId = new Guid("71000000-0000-0000-0000-000000000001"),
                            SumAssuredMax = 300000m,
                            SumAssuredMin = 300000m
                        },
                        new
                        {
                            Id = new Guid("71100000-0000-0000-0000-000000000014"),
                            AgeMax = 60,
                            AgeMin = 51,
                            AnnualPremium = 17800m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            ProductId = new Guid("71000000-0000-0000-0000-000000000001"),
                            SumAssuredMax = 500000m,
                            SumAssuredMin = 500000m
                        },
                        new
                        {
                            Id = new Guid("71100000-0000-0000-0000-000000000015"),
                            AgeMax = 60,
                            AgeMin = 51,
                            AnnualPremium = 26400m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            ProductId = new Guid("71000000-0000-0000-0000-000000000001"),
                            SumAssuredMax = 1000000m,
                            SumAssuredMin = 1000000m
                        },
                        new
                        {
                            Id = new Guid("71100000-0000-0000-0000-000000000016"),
                            AgeMax = 60,
                            AgeMin = 51,
                            AnnualPremium = 36900m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            ProductId = new Guid("71000000-0000-0000-0000-000000000001"),
                            SumAssuredMax = 1500000m,
                            SumAssuredMin = 1500000m
                        },
                        new
                        {
                            Id = new Guid("71100000-0000-0000-0000-000000000017"),
                            AgeMax = 30,
                            AgeMin = 18,
                            AnnualPremium = 3200m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            ProductId = new Guid("71000000-0000-0000-0000-000000000002"),
                            SumAssuredMax = 2500000m,
                            SumAssuredMin = 2500000m
                        },
                        new
                        {
                            Id = new Guid("71100000-0000-0000-0000-000000000018"),
                            AgeMax = 30,
                            AgeMin = 18,
                            AnnualPremium = 5800m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            ProductId = new Guid("71000000-0000-0000-0000-000000000002"),
                            SumAssuredMax = 5000000m,
                            SumAssuredMin = 5000000m
                        },
                        new
                        {
                            Id = new Guid("71100000-0000-0000-0000-000000000019"),
                            AgeMax = 30,
                            AgeMin = 18,
                            AnnualPremium = 8400m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            ProductId = new Guid("71000000-0000-0000-0000-000000000002"),
                            SumAssuredMax = 7500000m,
                            SumAssuredMin = 7500000m
                        },
                        new
                        {
                            Id = new Guid("71100000-0000-0000-0000-000000000020"),
                            AgeMax = 30,
                            AgeMin = 18,
                            AnnualPremium = 10900m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            ProductId = new Guid("71000000-0000-0000-0000-000000000002"),
                            SumAssuredMax = 10000000m,
                            SumAssuredMin = 10000000m
                        },
                        new
                        {
                            Id = new Guid("71100000-0000-0000-0000-000000000021"),
                            AgeMax = 40,
                            AgeMin = 31,
                            AnnualPremium = 4700m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            ProductId = new Guid("71000000-0000-0000-0000-000000000002"),
                            SumAssuredMax = 2500000m,
                            SumAssuredMin = 2500000m
                        },
                        new
                        {
                            Id = new Guid("71100000-0000-0000-0000-000000000022"),
                            AgeMax = 40,
                            AgeMin = 31,
                            AnnualPremium = 8700m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            ProductId = new Guid("71000000-0000-0000-0000-000000000002"),
                            SumAssuredMax = 5000000m,
                            SumAssuredMin = 5000000m
                        },
                        new
                        {
                            Id = new Guid("71100000-0000-0000-0000-000000000023"),
                            AgeMax = 40,
                            AgeMin = 31,
                            AnnualPremium = 12500m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            ProductId = new Guid("71000000-0000-0000-0000-000000000002"),
                            SumAssuredMax = 7500000m,
                            SumAssuredMin = 7500000m
                        },
                        new
                        {
                            Id = new Guid("71100000-0000-0000-0000-000000000024"),
                            AgeMax = 40,
                            AgeMin = 31,
                            AnnualPremium = 16200m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            ProductId = new Guid("71000000-0000-0000-0000-000000000002"),
                            SumAssuredMax = 10000000m,
                            SumAssuredMin = 10000000m
                        },
                        new
                        {
                            Id = new Guid("71100000-0000-0000-0000-000000000025"),
                            AgeMax = 50,
                            AgeMin = 41,
                            AnnualPremium = 8900m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            ProductId = new Guid("71000000-0000-0000-0000-000000000002"),
                            SumAssuredMax = 2500000m,
                            SumAssuredMin = 2500000m
                        },
                        new
                        {
                            Id = new Guid("71100000-0000-0000-0000-000000000026"),
                            AgeMax = 50,
                            AgeMin = 41,
                            AnnualPremium = 16600m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            ProductId = new Guid("71000000-0000-0000-0000-000000000002"),
                            SumAssuredMax = 5000000m,
                            SumAssuredMin = 5000000m
                        },
                        new
                        {
                            Id = new Guid("71100000-0000-0000-0000-000000000027"),
                            AgeMax = 50,
                            AgeMin = 41,
                            AnnualPremium = 23900m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            ProductId = new Guid("71000000-0000-0000-0000-000000000002"),
                            SumAssuredMax = 7500000m,
                            SumAssuredMin = 7500000m
                        },
                        new
                        {
                            Id = new Guid("71100000-0000-0000-0000-000000000028"),
                            AgeMax = 50,
                            AgeMin = 41,
                            AnnualPremium = 31200m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            ProductId = new Guid("71000000-0000-0000-0000-000000000002"),
                            SumAssuredMax = 10000000m,
                            SumAssuredMin = 10000000m
                        },
                        new
                        {
                            Id = new Guid("71100000-0000-0000-0000-000000000029"),
                            AgeMax = 55,
                            AgeMin = 51,
                            AnnualPremium = 14800m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            ProductId = new Guid("71000000-0000-0000-0000-000000000002"),
                            SumAssuredMax = 2500000m,
                            SumAssuredMin = 2500000m
                        },
                        new
                        {
                            Id = new Guid("71100000-0000-0000-0000-000000000030"),
                            AgeMax = 55,
                            AgeMin = 51,
                            AnnualPremium = 27700m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            ProductId = new Guid("71000000-0000-0000-0000-000000000002"),
                            SumAssuredMax = 5000000m,
                            SumAssuredMin = 5000000m
                        },
                        new
                        {
                            Id = new Guid("71100000-0000-0000-0000-000000000031"),
                            AgeMax = 55,
                            AgeMin = 51,
                            AnnualPremium = 39800m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            ProductId = new Guid("71000000-0000-0000-0000-000000000002"),
                            SumAssuredMax = 7500000m,
                            SumAssuredMin = 7500000m
                        },
                        new
                        {
                            Id = new Guid("71100000-0000-0000-0000-000000000032"),
                            AgeMax = 55,
                            AgeMin = 51,
                            AnnualPremium = 52100m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            ProductId = new Guid("71000000-0000-0000-0000-000000000002"),
                            SumAssuredMax = 10000000m,
                            SumAssuredMin = 10000000m
                        },
                        new
                        {
                            Id = new Guid("71100000-0000-0000-0000-000000000033"),
                            AgeMax = 75,
                            AgeMin = 18,
                            AnnualPremium = 7200m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            ProductId = new Guid("71000000-0000-0000-0000-000000000003"),
                            SumAssuredMax = 500000m,
                            SumAssuredMin = 100000m
                        },
                        new
                        {
                            Id = new Guid("71100000-0000-0000-0000-000000000034"),
                            AgeMax = 75,
                            AgeMin = 18,
                            AnnualPremium = 10900m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            ProductId = new Guid("71000000-0000-0000-0000-000000000003"),
                            SumAssuredMax = 1000000m,
                            SumAssuredMin = 500001m
                        },
                        new
                        {
                            Id = new Guid("71100000-0000-0000-0000-000000000035"),
                            AgeMax = 75,
                            AgeMin = 18,
                            AnnualPremium = 15700m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            ProductId = new Guid("71000000-0000-0000-0000-000000000003"),
                            SumAssuredMax = 1500000m,
                            SumAssuredMin = 1000001m
                        },
                        new
                        {
                            Id = new Guid("71100000-0000-0000-0000-000000000036"),
                            AgeMax = 75,
                            AgeMin = 18,
                            AnnualPremium = 22600m,
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 7, 19, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            ProductId = new Guid("71000000-0000-0000-0000-000000000003"),
                            SumAssuredMax = 2000000m,
                            SumAssuredMin = 1500001m
                        });
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.PremiumSchedule", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<decimal>("Amount")
                        .HasColumnType("decimal(10,2)")
                        .HasColumnName("amount");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<DateTime>("DueDate")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("due_date");

                    b.Property<int>("InstallmentNumber")
                        .HasColumnType("integer")
                        .HasColumnName("installment_number");

                    b.Property<Guid?>("PaymentId")
                        .HasColumnType("uuid")
                        .HasColumnName("payment_id");

                    b.Property<Guid?>("PolicyId")
                        .HasColumnType("uuid")
                        .HasColumnName("policy_id");

                    b.Property<Guid?>("ProposalId")
                        .HasColumnType("uuid")
                        .HasColumnName("proposal_id");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("status");

                    b.Property<DateTimeOffset?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.HasKey("Id")
                        .HasName("PK_premium_schedules");

                    b.HasIndex("PolicyId");

                    b.HasIndex("ProposalId");

                    b.ToTable("premium_schedules");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.ProcessedWebhookEvent", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("EventType")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("event_type");

                    b.Property<DateTimeOffset>("ProcessedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("processed_at");

                    b.Property<string>("StripeEventId")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)")
                        .HasColumnName("stripe_event_id");

                    b.HasKey("Id")
                        .HasName("PK_processed_webhook_events");

                    b.HasIndex("StripeEventId")
                        .IsUnique()
                        .HasDatabaseName("uq_processed_webhook_events_stripe_event_id");

                    b.ToTable("processed_webhook_events");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.ProductBrochure", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("BlobPath")
                        .IsRequired()
                        .HasMaxLength(1024)
                        .HasColumnType("character varying(1024)")
                        .HasColumnName("blob_path");

                    b.Property<int?>("ChildChunkCount")
                        .HasColumnType("integer")
                        .HasColumnName("child_chunk_count");

                    b.Property<string>("ContentHash")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("content_hash");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<Guid>("CreatedById")
                        .HasColumnType("uuid")
                        .HasColumnName("created_by_id");

                    b.Property<DateOnly>("EffectiveFrom")
                        .HasColumnType("date")
                        .HasColumnName("effective_from");

                    b.Property<DateOnly?>("EffectiveTo")
                        .HasColumnType("date")
                        .HasColumnName("effective_to");

                    b.Property<int?>("EmbeddingDimension")
                        .HasColumnType("integer")
                        .HasColumnName("embedding_dimension");

                    b.Property<string>("EmbeddingModel")
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)")
                        .HasColumnName("embedding_model");

                    b.Property<string>("EmbeddingProvider")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("embedding_provider");

                    b.Property<int>("FileSizeKb")
                        .HasColumnType("integer")
                        .HasColumnName("file_size_kb");

                    b.Property<string>("IngestionErrorCode")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("ingestion_error_code");

                    b.Property<string>("MimeType")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("mime_type");

                    b.Property<string>("OriginalFilename")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)")
                        .HasColumnName("original_filename");

                    b.Property<int?>("PageCount")
                        .HasColumnType("integer")
                        .HasColumnName("page_count");

                    b.Property<int?>("ParentChunkCount")
                        .HasColumnType("integer")
                        .HasColumnName("parent_chunk_count");

                    b.Property<Guid>("ProductId")
                        .HasColumnType("uuid")
                        .HasColumnName("product_id");

                    b.Property<DateTimeOffset?>("PublishedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("published_at");

                    b.Property<Guid?>("PublishedById")
                        .HasColumnType("uuid")
                        .HasColumnName("published_by_id");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(32)
                        .HasColumnType("character varying(32)")
                        .HasColumnName("status");

                    b.Property<string>("Version")
                        .IsRequired()
                        .HasMaxLength(32)
                        .HasColumnType("character varying(32)")
                        .HasColumnName("version");

                    b.HasKey("Id")
                        .HasName("PK_product_brochures");

                    b.HasIndex("CreatedById");

                    b.HasIndex("ProductId")
                        .IsUnique()
                        .HasDatabaseName("uq_product_brochures_current_published")
                        .HasFilter("\"status\" = 'Published'");

                    b.HasIndex("PublishedById");

                    b.HasIndex("ProductId", "ContentHash")
                        .IsUnique()
                        .HasDatabaseName("uq_product_brochures_product_content_hash");

                    b.HasIndex("ProductId", "Version")
                        .IsUnique()
                        .HasDatabaseName("uq_product_brochures_product_version");

                    b.ToTable("product_brochures");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.Proposal", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid?>("AgentId")
                        .HasColumnType("uuid")
                        .HasColumnName("agent_id");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<Guid>("CustomerId")
                        .HasColumnType("uuid")
                        .HasColumnName("customer_id");

                    b.Property<DateTimeOffset?>("DeletedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("deleted_at");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("boolean")
                        .HasColumnName("is_deleted");

                    b.Property<string>("PaymentFrequency")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("payment_frequency");

                    b.Property<string>("PolicyType")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("policy_type");

                    b.Property<decimal>("PremiumAmount")
                        .HasColumnType("decimal(10,2)")
                        .HasColumnName("premium_amount");

                    b.Property<Guid>("ProductId")
                        .HasColumnType("uuid")
                        .HasColumnName("product_id");

                    b.Property<string>("ProposalNumber")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("character varying(30)")
                        .HasColumnName("proposal_number");

                    b.Property<string>("RejectionReason")
                        .HasColumnType("text")
                        .HasColumnName("rejection_reason");

                    b.Property<DateTimeOffset?>("ReviewedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("reviewed_at");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("status");

                    b.Property<DateTimeOffset?>("SubmittedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("submitted_at");

                    b.Property<decimal>("SumAssured")
                        .HasColumnType("decimal(15,2)")
                        .HasColumnName("sum_assured");

                    b.Property<int>("TenureYears")
                        .HasColumnType("integer")
                        .HasColumnName("tenure_years");

                    b.Property<Guid?>("UnderwriterId")
                        .HasColumnType("uuid")
                        .HasColumnName("underwriter_id");

                    b.Property<string>("UnderwriterNotes")
                        .HasColumnType("text")
                        .HasColumnName("underwriter_notes");

                    b.Property<DateTimeOffset?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.HasKey("Id")
                        .HasName("PK_proposals");

                    b.HasIndex("AgentId");

                    b.HasIndex("CustomerId");

                    b.HasIndex("ProductId");

                    b.HasIndex("ProposalNumber")
                        .IsUnique()
                        .HasDatabaseName("uq_proposals_proposal_number");

                    b.HasIndex("UnderwriterId");

                    b.ToTable("proposals");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.ProposalMember", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<Guid>("CustomerMemberId")
                        .HasColumnType("uuid")
                        .HasColumnName("customer_member_id");

                    b.Property<Guid>("ProposalId")
                        .HasColumnType("uuid")
                        .HasColumnName("proposal_id");

                    b.HasKey("Id")
                        .HasName("PK_proposal_members");

                    b.HasIndex("CustomerMemberId");

                    b.HasIndex("ProposalId");

                    b.ToTable("proposal_members");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.Session", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<DateTime>("ExpiresAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("expires_at");

                    b.Property<string>("IpAddress")
                        .IsRequired()
                        .HasMaxLength(45)
                        .HasColumnType("character varying(45)")
                        .HasColumnName("ip_address");

                    b.Property<bool>("IsRevoked")
                        .HasColumnType("boolean")
                        .HasColumnName("is_revoked");

                    b.Property<string>("RefreshTokenHash")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("refresh_token_hash");

                    b.Property<DateTimeOffset?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<string>("UserAgent")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("user_agent");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid")
                        .HasColumnName("user_id");

                    b.HasKey("Id")
                        .HasName("PK_sessions");

                    b.HasIndex("UserId");

                    b.ToTable("sessions");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.SpeedyWorkspaceConversation", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<Guid>("CreatedByUserId")
                        .HasColumnType("uuid")
                        .HasColumnName("created_by_user_id");

                    b.Property<DateTimeOffset>("RetainUntil")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("retain_until");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("title");

                    b.Property<DateTimeOffset>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.HasKey("Id")
                        .HasName("PK_speedy_workspace_conversations");

                    b.HasIndex("RetainUntil")
                        .HasDatabaseName("ix_speedy_workspace_conversations_retain_until");

                    b.HasIndex("CreatedByUserId", "UpdatedAt")
                        .HasDatabaseName("ix_speedy_workspace_conversations_creator_updated");

                    b.ToTable("speedy_workspace_conversations");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.SpeedyWorkspaceMessage", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("ActionsJson")
                        .HasColumnType("text")
                        .HasColumnName("actions_json");

                    b.Property<string>("BrochureVersion")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("brochure_version");

                    b.Property<string>("CitationsJson")
                        .HasColumnType("text")
                        .HasColumnName("citations_json");

                    b.Property<string>("Content")
                        .IsRequired()
                        .HasMaxLength(8000)
                        .HasColumnType("character varying(8000)")
                        .HasColumnName("content");

                    b.Property<Guid>("ConversationId")
                        .HasColumnType("uuid")
                        .HasColumnName("conversation_id");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("EvidenceStatus")
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("evidence_status");

                    b.Property<string>("Intent")
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("intent");

                    b.Property<string>("Model")
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)")
                        .HasColumnName("model");

                    b.Property<string>("Risk")
                        .HasMaxLength(32)
                        .HasColumnType("character varying(32)")
                        .HasColumnName("risk");

                    b.Property<string>("Role")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("role");

                    b.HasKey("Id")
                        .HasName("PK_speedy_workspace_messages");

                    b.HasIndex("ConversationId", "CreatedAt")
                        .HasDatabaseName("ix_speedy_workspace_messages_conversation_created");

                    b.ToTable("speedy_workspace_messages");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.StripeCustomer", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("StripeCustomerId")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("stripe_customer_id");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid")
                        .HasColumnName("user_id");

                    b.HasKey("Id")
                        .HasName("PK_stripe_customers");

                    b.HasIndex("StripeCustomerId")
                        .IsUnique();

                    b.HasIndex("UserId")
                        .IsUnique();

                    b.ToTable("stripe_customers");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.SubmittedDocument", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("DocumentKey")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("document_key");

                    b.Property<Guid>("EntityId")
                        .HasColumnType("uuid")
                        .HasColumnName("entity_id");

                    b.Property<string>("EntityType")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("entity_type");

                    b.Property<string>("FilePath")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("file_path");

                    b.Property<int>("FileSizeKb")
                        .HasColumnType("integer")
                        .HasColumnName("file_size_kb");

                    b.Property<string>("MimeType")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("mime_type");

                    b.Property<string>("OriginalFilename")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)")
                        .HasColumnName("original_filename");

                    b.Property<string>("StoredFilename")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)")
                        .HasColumnName("stored_filename");

                    b.Property<DateTime>("UploadedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("uploaded_at");

                    b.Property<Guid>("UploadedBy")
                        .HasColumnType("uuid")
                        .HasColumnName("uploaded_by");

                    b.HasKey("Id")
                        .HasName("PK_submitted_documents");

                    b.ToTable("submitted_documents");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.Surveyor", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean")
                        .HasColumnName("is_active");

                    b.Property<DateOnly?>("LicenseExpiry")
                        .HasColumnType("date")
                        .HasColumnName("license_expiry");

                    b.Property<string>("LicenseNumber")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("license_number");

                    b.Property<string>("Specialization")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("specialization");

                    b.Property<string>("SurveyorType")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("surveyor_type");

                    b.Property<DateTimeOffset?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid")
                        .HasColumnName("user_id");

                    b.HasKey("Id")
                        .HasName("PK_surveyors");

                    b.HasIndex("UserId")
                        .IsUnique()
                        .HasDatabaseName("uq_surveyors_user_id");

                    b.ToTable("surveyors");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.SystemConfig", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("ConfigKey")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("config_key");

                    b.Property<string>("ConfigValue")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("config_value");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("description");

                    b.Property<DateTimeOffset?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<Guid?>("UpdatedById")
                        .HasColumnType("uuid")
                        .HasColumnName("updated_by_id");

                    b.HasKey("Id")
                        .HasName("PK_system_configs");

                    b.HasIndex("UpdatedById");

                    b.ToTable("system_configs");

                    b.HasData(
                        new
                        {
                            Id = new Guid("cc000001-0000-0000-0000-000000000001"),
                            ConfigKey = "AllowCustomerRegistration",
                            ConfigValue = "true",
                            Description = "Allow new customers to self-register via the portal. Set to false to disable public sign-up."
                        },
                        new
                        {
                            Id = new Guid("cc000002-0000-0000-0000-000000000002"),
                            ConfigKey = "PremiumGracePeriodDays",
                            ConfigValue = "30",
                            Description = "Number of days after the due date before a policy lapses for non-payment."
                        },
                        new
                        {
                            Id = new Guid("cc000003-0000-0000-0000-000000000003"),
                            ConfigKey = "ClaimApprovalThresholdInr",
                            ConfigValue = "100000",
                            Description = "Claims above this amount (INR) require senior underwriter sign-off before approval."
                        },
                        new
                        {
                            Id = new Guid("cc000004-0000-0000-0000-000000000004"),
                            ConfigKey = "KycVerificationRequired",
                            ConfigValue = "true",
                            Description = "Require completed KYC verification before a policy can be issued to a customer."
                        },
                        new
                        {
                            Id = new Guid("cc000005-0000-0000-0000-000000000005"),
                            ConfigKey = "MaxPremiumPaymentRetries",
                            ConfigValue = "3",
                            Description = "Maximum number of times a failed premium payment can be retried before the schedule is flagged."
                        });
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.User", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("AvatarUrl")
                        .HasColumnType("text")
                        .HasColumnName("avatar_url");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<DateTimeOffset?>("DeletedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("deleted_at");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)")
                        .HasColumnName("email");

                    b.Property<int>("FailedLoginAttempts")
                        .HasColumnType("integer")
                        .HasColumnName("failed_login_attempts");

                    b.Property<string>("FirstName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("first_name");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean")
                        .HasColumnName("is_active");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("boolean")
                        .HasColumnName("is_deleted");

                    b.Property<bool>("IsEmailVerified")
                        .HasColumnType("boolean")
                        .HasColumnName("is_email_verified");

                    b.Property<DateTime?>("LastLoginAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("last_login_at");

                    b.Property<string>("LastName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("last_name");

                    b.Property<DateTime?>("LockedUntil")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("locked_until");

                    b.Property<string>("PasswordHash")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("password_hash");

                    b.Property<string>("Phone")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasColumnName("phone");

                    b.Property<string>("Role")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("role");

                    b.Property<string>("Salutation")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)")
                        .HasColumnName("salutation");

                    b.Property<DateTimeOffset?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.HasKey("Id")
                        .HasName("PK_users");

                    b.HasIndex("Email")
                        .IsUnique()
                        .HasDatabaseName("uq_users_email");

                    b.HasIndex("Phone")
                        .IsUnique()
                        .HasDatabaseName("uq_users_phone");

                    b.ToTable("users");

                    b.HasData(
                        new
                        {
                            Id = new Guid("11111111-1111-1111-1111-111111111111"),
                            CreatedAt = new DateTimeOffset(new DateTime(2026, 6, 7, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)),
                            Email = "davish.std@gmail.com",
                            FailedLoginAttempts = 0,
                            FirstName = "Davish",
                            IsActive = true,
                            IsDeleted = false,
                            IsEmailVerified = true,
                            LastName = "Dev",
                            PasswordHash = "$2a$11$noTUuePjq5y/ldIskdG1JOVh7IxShG0RPMr3OEK8Mc6eXPKa3WTWK",
                            Phone = "9998887776",
                            Role = "Admin",
                            Salutation = "Mr"
                        });
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.UserConsent", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("ConsentType")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("consent_type");

                    b.Property<string>("ConsentVersion")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)")
                        .HasColumnName("consent_version");

                    b.Property<DateTimeOffset>("ConsentedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("consented_at");

                    b.Property<string>("IpAddress")
                        .HasMaxLength(45)
                        .HasColumnType("character varying(45)")
                        .HasColumnName("ip_address");

                    b.Property<bool>("IsGranted")
                        .HasColumnType("boolean")
                        .HasColumnName("is_granted");

                    b.Property<bool>("IsRevoked")
                        .HasColumnType("boolean")
                        .HasColumnName("is_revoked");

                    b.Property<DateTimeOffset?>("RevokedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("revoked_at");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid")
                        .HasColumnName("user_id");

                    b.HasKey("Id")
                        .HasName("PK_user_consents");

                    b.HasIndex("UserId", "ConsentType")
                        .HasDatabaseName("ix_user_consents_user_type");

                    b.ToTable("user_consents");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.UserToken", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<DateTime>("ExpiresAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("expires_at");

                    b.Property<bool>("IsUsed")
                        .HasColumnType("boolean")
                        .HasColumnName("is_used");

                    b.Property<string>("TokenHash")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("token_hash");

                    b.Property<string>("TokenType")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("token_type");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid")
                        .HasColumnName("user_id");

                    b.HasKey("Id")
                        .HasName("PK_user_tokens");

                    b.HasIndex("UserId");

                    b.ToTable("user_tokens");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.Address", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.User", "User")
                        .WithMany("Addresses")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_addresses_users_user_id");

                    b.Navigation("User");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.Agent", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.Branch", "Branch")
                        .WithMany()
                        .HasForeignKey("BranchId")
                        .HasConstraintName("FK_agents_branches_branch_id");

                    b.HasOne("SpeedClaim.Api.Models.User", "User")
                        .WithOne("Agent")
                        .HasForeignKey("SpeedClaim.Api.Models.Agent", "UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_agents_users_user_id");

                    b.Navigation("Branch");

                    b.Navigation("User");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.AgentCommission", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.Agent", "Agent")
                        .WithMany()
                        .HasForeignKey("AgentId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_agent_commissions_agents_agent_id");

                    b.HasOne("SpeedClaim.Api.Models.Policy", "Policy")
                        .WithMany()
                        .HasForeignKey("PolicyId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_agent_commissions_policies_policy_id");

                    b.HasOne("SpeedClaim.Api.Models.PremiumPayment", "PremiumPayment")
                        .WithMany()
                        .HasForeignKey("PremiumPaymentId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_agent_commissions_premium_payments_payment_id");

                    b.Navigation("Agent");

                    b.Navigation("Policy");

                    b.Navigation("PremiumPayment");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.AuditLog", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.User", "User")
                        .WithMany("AuditLogs")
                        .HasForeignKey("UserId");

                    b.Navigation("User");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.Claim", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.User", "AssignedOfficer")
                        .WithMany()
                        .HasForeignKey("AssignedOfficerId")
                        .OnDelete(DeleteBehavior.SetNull)
                        .HasConstraintName("FK_claims_users_assigned_officer_id");

                    b.HasOne("SpeedClaim.Api.Models.CustomerMember", "ClaimantMember")
                        .WithMany()
                        .HasForeignKey("ClaimantMemberId")
                        .OnDelete(DeleteBehavior.SetNull)
                        .HasConstraintName("FK_claims_customer_members_claimant_member_id");

                    b.HasOne("SpeedClaim.Api.Models.Customer", "Customer")
                        .WithMany("Claims")
                        .HasForeignKey("CustomerId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_claims_customers_customer_id");

                    b.HasOne("SpeedClaim.Api.Models.Policy", "Policy")
                        .WithMany("Claims")
                        .HasForeignKey("PolicyId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_claims_policies_policy_id");

                    b.HasOne("SpeedClaim.Api.Models.Surveyor", "Surveyor")
                        .WithMany("Claims")
                        .HasForeignKey("SurveyorId")
                        .OnDelete(DeleteBehavior.SetNull)
                        .HasConstraintName("FK_claims_surveyors");

                    b.Navigation("AssignedOfficer");

                    b.Navigation("ClaimantMember");

                    b.Navigation("Customer");

                    b.Navigation("Policy");

                    b.Navigation("Surveyor");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.ClaimStatusHistory", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.User", "ChangedBy")
                        .WithMany()
                        .HasForeignKey("ChangedById")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("FK_claim_status_history_users_changed_by_id");

                    b.HasOne("SpeedClaim.Api.Models.Claim", "Claim")
                        .WithMany("StatusHistory")
                        .HasForeignKey("ClaimId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_claim_status_history_claims_claim_id");

                    b.Navigation("ChangedBy");

                    b.Navigation("Claim");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.Customer", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.Agent", "OnboardingAgent")
                        .WithMany()
                        .HasForeignKey("OnboardingAgentId")
                        .OnDelete(DeleteBehavior.SetNull)
                        .HasConstraintName("FK_customers_agents_onboarding_agent_id");

                    b.HasOne("SpeedClaim.Api.Models.User", "User")
                        .WithOne("Customer")
                        .HasForeignKey("SpeedClaim.Api.Models.Customer", "UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_customers_users_user_id");

                    b.Navigation("OnboardingAgent");

                    b.Navigation("User");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.CustomerMember", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.Customer", "Customer")
                        .WithMany("CustomerMembers")
                        .HasForeignKey("CustomerId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_customer_members_customers_customer_id");

                    b.Navigation("Customer");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.DocumentRequirement", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.InsuranceProduct", "Product")
                        .WithMany()
                        .HasForeignKey("ProductId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .HasConstraintName("FK_document_requirements_insurance_products_ProductId");

                    b.Navigation("Product");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.EmailLog", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.User", "User")
                        .WithMany()
                        .HasForeignKey("UserId");

                    b.Navigation("User");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.Endorsement", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.Policy", "Policy")
                        .WithMany("Endorsements")
                        .HasForeignKey("PolicyId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_endorsements_policies_policy_id");

                    b.HasOne("SpeedClaim.Api.Models.User", "RequestedBy")
                        .WithMany()
                        .HasForeignKey("RequestedById")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_endorsements_users_requested_by");

                    b.HasOne("SpeedClaim.Api.Models.User", "ReviewedBy")
                        .WithMany()
                        .HasForeignKey("ReviewedById")
                        .OnDelete(DeleteBehavior.SetNull)
                        .HasConstraintName("FK_endorsements_users_reviewed_by");

                    b.Navigation("Policy");

                    b.Navigation("RequestedBy");

                    b.Navigation("ReviewedBy");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.ExternalIdentity", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.User", "User")
                        .WithMany("ExternalIdentities")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_external_identities_users_user_id");

                    b.Navigation("User");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.Grievance", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.User", "AssignedTo")
                        .WithMany()
                        .HasForeignKey("AssignedToId");

                    b.HasOne("SpeedClaim.Api.Models.Claim", "Claim")
                        .WithMany()
                        .HasForeignKey("ClaimId");

                    b.HasOne("SpeedClaim.Api.Models.Customer", "Customer")
                        .WithMany("Grievances")
                        .HasForeignKey("CustomerId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("SpeedClaim.Api.Models.Policy", "Policy")
                        .WithMany()
                        .HasForeignKey("PolicyId");

                    b.Navigation("AssignedTo");

                    b.Navigation("Claim");

                    b.Navigation("Customer");

                    b.Navigation("Policy");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.HealthClaimDetail", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.Claim", "Claim")
                        .WithOne("HealthDetail")
                        .HasForeignKey("SpeedClaim.Api.Models.HealthClaimDetail", "ClaimId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_health_claim_details_claims");

                    b.Navigation("Claim");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.HealthDetail", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.Policy", "Policy")
                        .WithOne("HealthDetail")
                        .HasForeignKey("SpeedClaim.Api.Models.HealthDetail", "PolicyId")
                        .OnDelete(DeleteBehavior.SetNull)
                        .HasConstraintName("FK_health_details_policies");

                    b.HasOne("SpeedClaim.Api.Models.Proposal", "Proposal")
                        .WithOne("HealthDetail")
                        .HasForeignKey("SpeedClaim.Api.Models.HealthDetail", "ProposalId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_health_details_proposals");

                    b.Navigation("Policy");

                    b.Navigation("Proposal");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.InsuranceProduct", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.User", "CreatedBy")
                        .WithMany()
                        .HasForeignKey("CreatedById");

                    b.Navigation("CreatedBy");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.KycRecord", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.User", "ReviewedBy")
                        .WithMany()
                        .HasForeignKey("ReviewedById");

                    b.HasOne("SpeedClaim.Api.Models.User", "User")
                        .WithOne("KycRecord")
                        .HasForeignKey("SpeedClaim.Api.Models.KycRecord", "UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_kyc_records_users_user_id");

                    b.Navigation("ReviewedBy");

                    b.Navigation("User");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.LifeClaimDetail", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.Claim", "Claim")
                        .WithOne("LifeDetail")
                        .HasForeignKey("SpeedClaim.Api.Models.LifeClaimDetail", "ClaimId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_life_claim_details_claims");

                    b.Navigation("Claim");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.LifeDetail", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.Policy", "Policy")
                        .WithOne("LifeDetail")
                        .HasForeignKey("SpeedClaim.Api.Models.LifeDetail", "PolicyId")
                        .OnDelete(DeleteBehavior.SetNull)
                        .HasConstraintName("FK_life_details_policies");

                    b.HasOne("SpeedClaim.Api.Models.Proposal", "Proposal")
                        .WithOne("LifeDetail")
                        .HasForeignKey("SpeedClaim.Api.Models.LifeDetail", "ProposalId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_life_details_proposals");

                    b.Navigation("Policy");

                    b.Navigation("Proposal");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.MotorClaimDetail", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.Claim", "Claim")
                        .WithOne("MotorDetail")
                        .HasForeignKey("SpeedClaim.Api.Models.MotorClaimDetail", "ClaimId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_motor_claim_details_claims");

                    b.Navigation("Claim");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.MotorDetail", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.Policy", "Policy")
                        .WithOne("MotorDetail")
                        .HasForeignKey("SpeedClaim.Api.Models.MotorDetail", "PolicyId")
                        .OnDelete(DeleteBehavior.SetNull)
                        .HasConstraintName("FK_motor_details_policies");

                    b.HasOne("SpeedClaim.Api.Models.Proposal", "Proposal")
                        .WithOne("MotorDetail")
                        .HasForeignKey("SpeedClaim.Api.Models.MotorDetail", "ProposalId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_motor_details_proposals");

                    b.Navigation("Policy");

                    b.Navigation("Proposal");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.Nominee", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.Policy", "Policy")
                        .WithMany("Nominees")
                        .HasForeignKey("PolicyId")
                        .OnDelete(DeleteBehavior.SetNull)
                        .HasConstraintName("FK_nominees_policies");

                    b.HasOne("SpeedClaim.Api.Models.Proposal", "Proposal")
                        .WithMany("Nominees")
                        .HasForeignKey("ProposalId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_nominees_proposals");

                    b.Navigation("Policy");

                    b.Navigation("Proposal");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.Notification", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.User", "User")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.Policy", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.Agent", "Agent")
                        .WithMany()
                        .HasForeignKey("AgentId")
                        .OnDelete(DeleteBehavior.SetNull)
                        .HasConstraintName("FK_policies_agents_agent_id");

                    b.HasOne("SpeedClaim.Api.Models.Customer", "Customer")
                        .WithMany("Policies")
                        .HasForeignKey("CustomerId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_policies_customers_customer_id");

                    b.HasOne("SpeedClaim.Api.Models.ProductBrochure", "ProductBrochure")
                        .WithMany()
                        .HasForeignKey("ProductBrochureId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("FK_policies_product_brochures_product_brochure_id");

                    b.HasOne("SpeedClaim.Api.Models.InsuranceProduct", "Product")
                        .WithMany()
                        .HasForeignKey("ProductId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_policies_products_product_id");

                    b.HasOne("SpeedClaim.Api.Models.Proposal", "Proposal")
                        .WithOne()
                        .HasForeignKey("SpeedClaim.Api.Models.Policy", "ProposalId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("FK_policies_proposals_proposal_id");

                    b.Navigation("Agent");

                    b.Navigation("Customer");

                    b.Navigation("Product");

                    b.Navigation("ProductBrochure");

                    b.Navigation("Proposal");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.PolicyAssistantConversation", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.ProductBrochure", "Brochure")
                        .WithMany()
                        .HasForeignKey("BrochureId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_policy_assistant_conversations_product_brochures_brochure_id");

                    b.HasOne("SpeedClaim.Api.Models.User", "CreatedBy")
                        .WithMany()
                        .HasForeignKey("CreatedByUserId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_policy_assistant_conversations_users_created_by_id");

                    b.HasOne("SpeedClaim.Api.Models.Policy", "Policy")
                        .WithMany("AssistantConversations")
                        .HasForeignKey("PolicyId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_policy_assistant_conversations_policies_policy_id");

                    b.Navigation("Brochure");

                    b.Navigation("CreatedBy");

                    b.Navigation("Policy");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.PolicyAssistantMessage", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.PolicyAssistantConversation", "Conversation")
                        .WithMany("Messages")
                        .HasForeignKey("ConversationId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_policy_assistant_messages_conversations_conversation_id");

                    b.Navigation("Conversation");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.PolicyMember", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.CustomerMember", "CustomerMember")
                        .WithMany()
                        .HasForeignKey("CustomerMemberId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("SpeedClaim.Api.Models.Policy", "Policy")
                        .WithMany("PolicyMembers")
                        .HasForeignKey("PolicyId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_policy_members_policies");

                    b.Navigation("CustomerMember");

                    b.Navigation("Policy");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.PolicyStatusHistory", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.User", "ChangedBy")
                        .WithMany()
                        .HasForeignKey("ChangedById")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("FK_policy_status_history_users_changed_by_id");

                    b.HasOne("SpeedClaim.Api.Models.Policy", "Policy")
                        .WithMany("StatusHistory")
                        .HasForeignKey("PolicyId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_policy_status_history_policies_policy_id");

                    b.Navigation("ChangedBy");

                    b.Navigation("Policy");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.PremiumPayment", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.Customer", "Customer")
                        .WithMany("Payments")
                        .HasForeignKey("CustomerId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("SpeedClaim.Api.Models.Policy", "Policy")
                        .WithMany("PremiumPayments")
                        .HasForeignKey("PolicyId")
                        .OnDelete(DeleteBehavior.SetNull)
                        .HasConstraintName("FK_premium_payments_policies");

                    b.HasOne("SpeedClaim.Api.Models.Proposal", "Proposal")
                        .WithMany("PremiumPayments")
                        .HasForeignKey("ProposalId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .HasConstraintName("FK_premium_payments_proposals");

                    b.HasOne("SpeedClaim.Api.Models.PremiumSchedule", "Schedule")
                        .WithOne("Payment")
                        .HasForeignKey("SpeedClaim.Api.Models.PremiumPayment", "ScheduleId")
                        .HasConstraintName("FK_premium_payments_schedules");

                    b.Navigation("Customer");

                    b.Navigation("Policy");

                    b.Navigation("Proposal");

                    b.Navigation("Schedule");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.PremiumRateTable", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.InsuranceProduct", "Product")
                        .WithMany()
                        .HasForeignKey("ProductId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_premium_rate_tables_products");

                    b.Navigation("Product");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.PremiumSchedule", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.Policy", "Policy")
                        .WithMany("PremiumSchedules")
                        .HasForeignKey("PolicyId")
                        .OnDelete(DeleteBehavior.SetNull)
                        .HasConstraintName("FK_premium_schedules_policies");

                    b.HasOne("SpeedClaim.Api.Models.Proposal", "Proposal")
                        .WithMany("PremiumSchedules")
                        .HasForeignKey("ProposalId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .HasConstraintName("FK_premium_schedules_proposals");

                    b.Navigation("Policy");

                    b.Navigation("Proposal");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.ProductBrochure", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.User", "CreatedBy")
                        .WithMany()
                        .HasForeignKey("CreatedById")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_product_brochures_users_created_by_id");

                    b.HasOne("SpeedClaim.Api.Models.InsuranceProduct", "Product")
                        .WithMany("Brochures")
                        .HasForeignKey("ProductId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_product_brochures_products_product_id");

                    b.HasOne("SpeedClaim.Api.Models.User", "PublishedBy")
                        .WithMany()
                        .HasForeignKey("PublishedById")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("FK_product_brochures_users_published_by_id");

                    b.Navigation("CreatedBy");

                    b.Navigation("Product");

                    b.Navigation("PublishedBy");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.Proposal", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.Agent", "Agent")
                        .WithMany()
                        .HasForeignKey("AgentId")
                        .OnDelete(DeleteBehavior.SetNull)
                        .HasConstraintName("FK_proposals_agents_agent_id");

                    b.HasOne("SpeedClaim.Api.Models.Customer", "Customer")
                        .WithMany("Proposals")
                        .HasForeignKey("CustomerId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_proposals_customers_customer_id");

                    b.HasOne("SpeedClaim.Api.Models.InsuranceProduct", "Product")
                        .WithMany()
                        .HasForeignKey("ProductId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_proposals_products_product_id");

                    b.HasOne("SpeedClaim.Api.Models.User", "Underwriter")
                        .WithMany()
                        .HasForeignKey("UnderwriterId")
                        .OnDelete(DeleteBehavior.SetNull)
                        .HasConstraintName("FK_proposals_users_underwriter_id");

                    b.Navigation("Agent");

                    b.Navigation("Customer");

                    b.Navigation("Product");

                    b.Navigation("Underwriter");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.ProposalMember", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.CustomerMember", "CustomerMember")
                        .WithMany()
                        .HasForeignKey("CustomerMemberId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_proposal_members_customer_members_customer_member_id");

                    b.HasOne("SpeedClaim.Api.Models.Proposal", "Proposal")
                        .WithMany("ProposalMembers")
                        .HasForeignKey("ProposalId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_proposal_members_proposals_proposal_id");

                    b.Navigation("CustomerMember");

                    b.Navigation("Proposal");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.Session", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.User", "User")
                        .WithMany("Sessions")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_sessions_users_user_id");

                    b.Navigation("User");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.SpeedyWorkspaceConversation", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.User", "CreatedBy")
                        .WithMany()
                        .HasForeignKey("CreatedByUserId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_speedy_workspace_conversations_users_created_by_id");

                    b.Navigation("CreatedBy");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.SpeedyWorkspaceMessage", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.SpeedyWorkspaceConversation", "Conversation")
                        .WithMany("Messages")
                        .HasForeignKey("ConversationId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_speedy_workspace_messages_conversations_conversation_id");

                    b.Navigation("Conversation");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.StripeCustomer", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.User", "User")
                        .WithOne()
                        .HasForeignKey("SpeedClaim.Api.Models.StripeCustomer", "UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_stripe_customers_users_user_id");

                    b.Navigation("User");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.Surveyor", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.User", "User")
                        .WithOne("Surveyor")
                        .HasForeignKey("SpeedClaim.Api.Models.Surveyor", "UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_surveyors_users_user_id");

                    b.Navigation("User");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.SystemConfig", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.User", "UpdatedBy")
                        .WithMany()
                        .HasForeignKey("UpdatedById");

                    b.Navigation("UpdatedBy");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.UserConsent", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.User", "User")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.UserToken", b =>
                {
                    b.HasOne("SpeedClaim.Api.Models.User", "User")
                        .WithMany("UserTokens")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_user_tokens_users_user_id");

                    b.Navigation("User");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.Claim", b =>
                {
                    b.Navigation("HealthDetail");

                    b.Navigation("LifeDetail");

                    b.Navigation("MotorDetail");

                    b.Navigation("StatusHistory");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.Customer", b =>
                {
                    b.Navigation("Claims");

                    b.Navigation("CustomerMembers");

                    b.Navigation("Grievances");

                    b.Navigation("Payments");

                    b.Navigation("Policies");

                    b.Navigation("Proposals");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.InsuranceProduct", b =>
                {
                    b.Navigation("Brochures");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.Policy", b =>
                {
                    b.Navigation("AssistantConversations");

                    b.Navigation("Claims");

                    b.Navigation("Endorsements");

                    b.Navigation("HealthDetail");

                    b.Navigation("LifeDetail");

                    b.Navigation("MotorDetail");

                    b.Navigation("Nominees");

                    b.Navigation("PolicyMembers");

                    b.Navigation("PremiumPayments");

                    b.Navigation("PremiumSchedules");

                    b.Navigation("StatusHistory");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.PolicyAssistantConversation", b =>
                {
                    b.Navigation("Messages");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.PremiumSchedule", b =>
                {
                    b.Navigation("Payment");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.Proposal", b =>
                {
                    b.Navigation("HealthDetail");

                    b.Navigation("LifeDetail");

                    b.Navigation("MotorDetail");

                    b.Navigation("Nominees");

                    b.Navigation("PremiumPayments");

                    b.Navigation("PremiumSchedules");

                    b.Navigation("ProposalMembers");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.SpeedyWorkspaceConversation", b =>
                {
                    b.Navigation("Messages");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.Surveyor", b =>
                {
                    b.Navigation("Claims");
                });

            modelBuilder.Entity("SpeedClaim.Api.Models.User", b =>
                {
                    b.Navigation("Addresses");

                    b.Navigation("Agent");

                    b.Navigation("AuditLogs");

                    b.Navigation("Customer");

                    b.Navigation("ExternalIdentities");

                    b.Navigation("KycRecord");

                    b.Navigation("Sessions");

                    b.Navigation("Surveyor");

                    b.Navigation("UserTokens");
                });
#pragma warning restore 612, 618
        }
    }
}

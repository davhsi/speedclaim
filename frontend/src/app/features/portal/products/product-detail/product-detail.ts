...
@Component({
  ...
  standalone: true,
  imports: [RouterLink, SafeHtmlPipe, SkeletonLoaderComponent],
...
export class ProductDetailComponent implements OnInit {
  ...
  coverLabel(product: ProductDto): string {
    if (product.domain.toUpperCase() === 'HEALTH') return 'Cover options';
    if (product.domain.toUpperCase() === 'MOTOR') return 'Vehicle IDV';
    return 'Sum assured';
  }

  coverValue(product: ProductDto): string {
    if (product.domain.toUpperCase() === 'MOTOR') return 'Calculated from vehicle value';
    if (product.domain.toUpperCase() === 'HEALTH') return (product.coverageOptions ?? []).map(value => `₹${value.toLocaleString('en-IN')}`).join(' · ');
    return `${product.minSumAssured.toLocaleString('en-IN')}–${product.maxSumAssured.toLocaleString('en-IN')} in ₹${(product.sumAssuredIncrement ?? 1).toLocaleString('en-IN')} steps`;
  }
}

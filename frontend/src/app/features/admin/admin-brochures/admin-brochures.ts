...
export class AdminBrochuresComponent implements OnInit {
  ...
  onFile(event: Event): void { const file = (event.target as HTMLInputElement).files?.[0] ?? null; this.selectedFile.set(file?.type === 'application/pdf' || file?.name.toLowerCase().endsWith('.pdf') ? file : null); if (file && !this.selectedFile()) this.toast.error('Select a PDF brochure.'); }
  upload(): void { const file = this.selectedFile(), product = this.selectedProductId(); if (!file || !product || this.uploading()) return; this.uploading.set(true); this.admin.uploadProductBrochure(product, file, this.effectiveFrom, this.version).subscribe({ next: x => { this.brochures.update(items => [x, ...items]); this.selectedFile.set(null); this.version = ''; this.uploading.set(false); this.toast.success('Brochure uploaded and indexed.'); }, error: err => { this.uploading.set(false); this.toast.error(err?.error?.message ?? 'Brochure upload failed'); } }); }
  update(action: 'publish' | 'archive' | 'retry', brochure: ProductBrochureDto): void { const product = this.selectedProductId(); const request = action === 'publish' ? this.admin.publishProductBrochure(product, brochure.id) : action === 'archive' ? this.admin.archiveProductBrochure(product, brochure.id) : this.admin.retryProductBrochure(product, brochure.id); request.subscribe({ next: updated => { this.brochures.update(items => items.map(x => x.id === updated.id ? updated : x)); this.toast.success(`Brochure ${action}d.`); }, error: err => this.toast.error(err?.error?.message ?? `Could not ${action} brochure`) }); }
